...
test.describe("Tool Filter Runtime Behavior", () => {
  test("--tool-filter core exposes only core + codemode tools", async () => {
    ...
    try {
      ...
      try {
        ...
        expect(names).toContain("mysql_read_query");
        expect(names).toContain("mysql_list_tables");
        expect(names).toContain("mysql_execute_code");

        // Should NOT contain tools from other groups
        expect(names).not.toContain("mysql_jsonb_extract");
        expect(names).not.toContain("mysql_vector_search");
        expect(names).not.toContain("mysql_text_search");
        expect(names).not.toContain("mysql_transaction_begin");
      } finally {
        await client.close();
      }
    } finally {
  ...
  test("--tool-filter core,-codemode excludes mysql_execute_code", async () => {
    ...
    try {
      ...
      try {
        ...
        expect(names).toContain("mysql_read_query");
        expect(names).not.toContain("mysql_execute_code");
      } finally {
        await client.close();
      }
    } finally {
  ...
  test("--tool-filter with individual tools exposes exact whitelist", async () => {
    ...
    await startServer(
      port,
      ["--tool-filter", "mysql_read_query,mysql_list_tables"],
      "filter-whitelist",
    );
