...
test.describe.serial("Advanced Session Management", () => {
  test.skip("should reject SSE session ID on /mcp (cross-protocol guard)", async ({}, testInfo) => {
  ...
  test.skip("should support sequential session isolation", async ({}, testInfo) => {
    const baseURL = process.env.MCP_TEST_URL || "http://127.0.0.1:3000";
  ...
  test.skip("should reject request with non-existent session ID", async ({}, testInfo) => {
    const baseURL = process.env.MCP_TEST_URL || "http://127.0.0.1:3000";
