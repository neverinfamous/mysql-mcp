...
test.describe("Payload Contracts: Spatial", () => {
  test("mysql_spatial_distance returns { results[], count, referencePoint }", async () => {
    ...
    try {
      ...
      expect(Array.isArray((payload.data as Record<string, unknown>).results)).toBe(true);
      expect(typeof (payload.data as Record<string, unknown>).count).toBe("number");
      expect(typeof (payload.data as Record<string, unknown>).referencePoint).toBe("object");
    } finally {
      await client.close();
    }
  });
  ...
  test("mysql_spatial_distance_sphere returns { results[], count, unit }", async () => {
    ...
    try {
      ...
      expect(Array.isArray((payload.data as Record<string, unknown>).results)).toBe(true);
      expect(typeof (payload.data as Record<string, unknown>).count).toBe("number");
      expect((payload.data as Record<string, unknown>).unit).toBe("meters");
    } finally {
      await client.close();
    }
  });
