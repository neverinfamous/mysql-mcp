...
test.describe("Integration: Core + Stats Cross-Validation", () => {
  test.skip("manual COUNT matches count tool", async ({}, testInfo) => {
    const client = await createClient();
    try {
      const p = await callToolAndParse(client, "mysql_execute_code", {
        code: `
          const table = "test_products";
          const manual = await mysql.core.readQuery({ query: "SELECT COUNT(*) as cnt FROM " + table });
          const countResult = await mysql.core.count({ table });
          return {
      ...
      expectSuccess(p);
      const result = p.result as Record<string, unknown>;
      // COUNT returns bigint (string), mysql_count returns number — compare as numbers
...
test.describe.skip("Integration: Data Integrity", () => {
  test.skip("test_products still has rows", async ({}, testInfo) => {
    const client = await createClient();
    try {
      const p = await callToolAndParse(client, "mysql_count", {
        table: "test_products",
      });
      expectSuccess(p);
  ...
  test.skip("test_orders still has rows", async ({}, testInfo) => {
    const client = await createClient();
    try {
      const p = await callToolAndParse(client, "mysql_count", {
        table: "test_orders",
