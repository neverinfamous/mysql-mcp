...
test.describe("Payload Contracts: Misc", () => {
  ...
  test("mysql_transaction_begin + rollback lifecycle", async () => {
    ...
    try {
      ...
      expect(typeof (beginPayload.data as Record<string, unknown>).transactionId).toBe("string");

      const rollbackPayload = await callToolAndParse(
        client,
        "mysql_transaction_rollback",
        { transactionId: (beginPayload.data as Record<string, unknown>).transactionId as string },
      );
