...
- **New Parameters** — `updateServerSettings`, `ddlOnly`, and `all` for shell/backup tools
...
- **112 New Tests** targeting uncovered branches across multiple modules:
  - **CLI** — `canSkipMySQLConnection()` covering router-only, proxysql-only, shell-only, ecosystem shortcuts
  - **Shell Types** — 100% branch coverage for `booleanCoerce` preprocessor
  - **Data Transfer** — Tests for `updateServerSettings`, `local_infile` error handling, X Protocol
  - **Backup/Restore** — Tests for `ddlOnly` mode, privilege errors, fatal dump errors
  - **JSON Enhanced** — Tests for merge, diff, type mappings, cardinality filtering
  - **InnoDB Cluster** — New test file covering fallback to GR status, error handling

- **Branch Coverage** — Improved from ~83% to ~86%
- **Total Tests** — 1590 tests passing across 101 test files
