...
- **Code Mode Engine Hardening**: Added V8 code generation restrictions (`strings: false`, `wasm: false`) and sandbox prototype freezing for defense-in-depth isolation.
- **Global Stdio CLI Fix**: Resolved an issue where the MCP Server would hang indefinitely on startup when installed globally via npm.
- **Docker CI/CD Verification**: Added robust HTTP transport and binary loading smoke tests to both PR and release publishing workflows.
- **Docs Standardization**: Brought README layout, metrics, and badges up to fleet standards, and rewrote CONTRIBUTING.md.
...
```bash
npx @neverinfamous/mysql-mcp@latest
```
