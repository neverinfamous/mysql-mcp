...
description: "Delivers 37 new features—including Vector and Audit tool groups—alongside over 500 fixes to enhance security and code mode reliability."
---
...
- **New Vector Search Tool Group**: Unlocks MySQL 9.0+ capabilities with dedicated tools for vector and hybrid search operations.
- **Audit & Optimization Engines**: Introduces `mysql_audit_search` and the Q5 Index Recommendation engine to improve automated database analysis.
- **Code Mode Sandbox Integration**: Ports `isolated-vm` from `db-mcp` to enable secure arbitrary code execution in a controlled sandbox environment.
- **Security Posture**: Resolves identified vulnerabilities through updated deployment workflows and strict tool input validation.
- **Resilient Transport Layer**: Hardens the Streamable HTTP layer against context deadline issues and session terminations to improve connection reliability.
