...
description: "Release notes for mysql-mcp v3.1.0"
---

# mysql-mcp v3.1.0

## Highlights
- **Gain Deep Database Insights**: Assess schema risk and map dependencies effortlessly with the new Introspection tool group.
- **Supercharge Developer Velocity**: Execute dynamic schema updates, track migrations, and instantly revert changes directly from the AI agent.
- **Prevent OOM Crashes**: Safeguard against context window exhaustion with strict egress boundaries and heuristic token estimation.
- **Ensure Bulletproof Transports**: Support a wider range of enterprise clients with simultaneous Streamable HTTP and Legacy SSE protocols.
...
```bash
npm install -g @neverinfamous/mysql-mcp@3.1.0
docker pull writenotenow/mysql-mcp:v3.1.0
```

[Full Changelog](https://github.com/neverinfamous/mysql-mcp/compare/v3.0.2...v3.1.0)
