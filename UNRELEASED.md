...
- **Tests**: Remediated benchmark timing assertions, fixed `vitest bench` watch-mode hangs, and gracefully skipped E2E write tests in read-only mode. Certified `replication` and `roles` tool group payload boundaries and single-server resilience.
