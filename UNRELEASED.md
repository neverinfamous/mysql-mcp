...
- Fixed spatial tool group WKT round-tripping for SRID 4326 by explicitly requesting `'axis-order=long-lat'` in all `ST_AsText` output queries in `geometry.ts`, `queries.ts`, and `operations.ts`. Certified spatial tool group via advanced Code Mode stress testing, validating boundary coordinates, geometry operations, and SRID mismatched payload constraints.
