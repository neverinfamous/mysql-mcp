...
- Fixed schema tool group error handling by enforcing `{ success: true }` for all handlers and replacing legacy existence checks (e.g. `{ exists: false }`) and skips (e.g. `{ skipped: true }`) with standard `ErrorResponse` domain errors. Refactored tool Zod schemas to support common parameter aliases (`database` mapping to `schema`, `query` mapping to `definition`). Remediated associated unit test assertions and resolved ESLint `strict-boolean-expressions` and type casting violations.
## Security
