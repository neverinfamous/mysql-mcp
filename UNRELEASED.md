...
- Fixed `performance` anomaly tools by updating input validation to emit strict structured errors for out-of-bounds parameters and accurately reflect alias usage (`minExecutions`, `thresholdPercent`). Certified `performance` tool group via advanced Code Mode stress testing, validating architectural integrity of `EXPLAIN` payload sizes and summary mode efficiency.
