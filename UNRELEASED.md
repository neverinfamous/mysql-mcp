...
- **JSON**: Fixed parameter visibility in `json_validate` and handled malformed JSON/empty strings gracefully. Added `path` alias for `paths` in `json_remove`. Implemented missing `where` and `limit` clauses for `json_contains` and `json_keys`. Added missing `metrics.tokenEstimate` payloads across all operations, standardized domain error wrapping via `formatHandlerErrorResponse`, and aligned `json_get` and `json_update` schemas to use the standard `where` clause instead of `id`/`idColumn`. Added `limit` support to `json_extract` to strictly bound result set payload token consumption.
