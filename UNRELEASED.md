...
- **Core**: Remediated missing `metrics.tokenEstimate` payloads in success and domain error paths across all core tools (`mysql_read_query`, `mysql_write_query`, `mysql_list_tables`, `mysql_describe_table`, `mysql_create_table`, `mysql_drop_table`, `mysql_get_indexes`, `mysql_create_index`) by integrating `withTokenEstimate` wrappers. Certified Advanced Stress Tests (Category 1-4) under Code Mode with 100% compliance.
...
- **Spatial**: Fixed `spatial_create_index` emitting table not found errors on missing columns. Fixed WKT round-tripping for SRID 4326. Optimized `mysql_spatial_buffer` payload by removing massive GeoJSON generation. Added missing `metrics.tokenEstimate` payloads to all success and error paths.
