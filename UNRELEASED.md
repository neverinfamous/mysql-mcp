...
- Fixed `performance` anomaly tools by adding `stdDevThreshold` alias to `mysql_detect_query_anomalies` and `windowMinutes` parameter validation bounds to `mysql_detect_connection_spike`, ensuring strict structured error responses for invalid thresholds instead of swallowing the payload. Certified `performance` tool group via advanced Code Mode stress testing.
