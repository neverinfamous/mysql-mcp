...
- **Tests**: Remediated benchmark timing assertions, fixed `vitest bench` watch-mode hangs, and gracefully skipped E2E write tests in read-only mode. Certified `replication` and `transactions` tool groups for payload boundaries, structured error contracts, rollback bounds, and parameter alias parity.
