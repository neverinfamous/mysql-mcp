...
- Certified `proxysql` tool group for production readiness by completing advanced Code Mode stress tests. Optimized token payload efficiency by defaulting `proxysql_status` and `proxysql_runtime_status` to `summary: true`, and reduced default limits across all list tools (down to 10 or 20) to strictly prevent token payloads from exceeding 500 tokens. Updated corresponding unit test assertions.
