...
- **Admin Domain Errors Structured Format**: Modified the domain error extraction in the Admin tool group (via `extractMaintenanceError`, `flushTables`, and `killQuery`) to return the fully populated `ErrorResponse` structure (`code`, `category`, `recoverable`, `details`). Previously, domain errors returned partial objects missing these fields, causing type-compliance issues with the system's central error interfaces. Verified 100% adherence via exhaustive Code Mode re-testing across happy paths, domain errors, and Zod validations.
