...
- **Cluster**: Certified full tool group stability during production verification. All 10 operations (`mysql_gr_status`, `mysql_cluster_status`, `mysql_cluster_topology`, etc.) returned correct structured responses with efficient payload token usage. Fixed auto-recovery by changing `group_replication_start_on_boot=ON` to persist state across restarts. Merged contextual fallback strings into standard `error` properties for multiple failure scenarios in instance and router status tools.
...
- **Sys Schema**: Applied Split Schema pattern to `mysql_sys_statement_summary`, `mysql_sys_wait_summary`, `mysql_sys_io_summary`, `mysql_sys_schema_stats`, `mysql_sys_user_summary`, and `mysql_sys_host_summary` to enable robust Zod type coercion for the `limit` parameter, ensuring invalid numeric types return structured handler errors instead of raw MCP exceptions.
