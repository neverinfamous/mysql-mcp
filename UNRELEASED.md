...
- **Tests**: Remediated benchmark timing assertions, fixed `vitest bench` watch-mode hangs, and gracefully skipped E2E write tests in read-only mode. Certified `replication` tool group payload boundaries and single-server resilience. Certified `schema` tool group DDL idempotency and cross-object dependency handling.
