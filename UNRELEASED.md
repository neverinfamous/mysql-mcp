...
- **Router**: Fixed `router_route_health` to gracefully return `{ success: true, health: { isAlive: false } }` on 500 errors for offline routes. Applied Split Schema pattern to all router schemas (`mysql_router_route_status`, `mysql_router_route_health`, `mysql_router_route_connections`, etc.) to enable robust Zod validation and type coercion, ensuring missing or empty required parameters return structured handler errors instead of raw MCP exceptions.
