# mysql-mcp Usability & Hallucination Test: Backup
...
`export_table`, `import_data`, `create_dump`, `restore_dump`, `audit_list_backups`, `audit_diff_backup`, `audit_restore_backup`
...
- Test method naming aliases: See if `mysql.backup.dump()` or `mysql.backup.export()` work.
...
1. If local verification passes, run `git add .` and `git commit -m "Optimize backup tool usage"`.
