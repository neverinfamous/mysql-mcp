# mysql-mcp Usability & Hallucination Test: Introspection & Migration

> **This test is optimized for an autonomous agent.**

This prompt instructs you to organically test the `introspection` and `migration` tool groups using Code Mode (`mysql_execute_code`), intentionally fuzzing the inputs to discover agent hallucinations, and permanently hardening the codebase against them.

## 1. Fuzz Phase

Use the `mysql_execute_code` tool to interact with the following tools across the two groups:
`dependency_graph`, `topological_sort`, `cascade_simulator`, `constraint_analysis`, `migration_risks`, `schema_snapshot`, `migration_init`, `migration_record`, `migration_apply`, `migration_rollback`, `migration_history`, `migration_status`
...
- Test positional params and aliases: Try `mysql.introspection.graph()` or similar intuitive names.
...
1. If local verification passes, run `git add .` and `git commit -m "Optimize introspection and migration tool usage"`.
