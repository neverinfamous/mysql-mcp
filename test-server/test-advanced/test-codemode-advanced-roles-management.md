...
2. `mysql_role_grants({role: "stress_role_a"})` → verify SELECT is listed
3. `mysql_role_grant({role: "stress_role_a", privilege: "INSERT", on: "testdb.*"})` → success
4. `mysql_role_grants({role: "stress_role_a"})` → verify both SELECT and INSERT are listed
5. `mysql_role_revoke({role: "stress_role_a", privilege: "SELECT", on: "testdb.*"})` → success
6. `mysql_role_grants({role: "stress_role_a"})` → verify SELECT is removed, INSERT remains
