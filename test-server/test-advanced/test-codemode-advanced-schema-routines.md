...
1. `mysql.schema.listStoredProcedures` with `database` param → verify response matches `schema` param
2. `mysql.schema.listFunctions` with `database` param → verify response matches `schema` param
