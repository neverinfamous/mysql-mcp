# mysql-mcp Advanced Stress Testing: [schema-routines]
...
2. **Error Path Testing**: For **every** tool, test at least **two** invalid inputs:
   ...
   - (b) An **empty parameters test** (call the tool with `{}`) — provided the tool has required parameters. (If all parameters are inherently optional, verify it succeeds).
     Invalid inputs must return a **structured handler error** (`{success: false, error: "..."}`) — NOT a raw MCP error frame.
...
5. **Commit**: Stage and commit all changes — do NOT push. **CRITICAL**: Your commit message MUST explicitly include the name of this tool group prompt file (e.g. `[Testing: test-codemode-advanced-schema-routines.md]`) so the history can be traced.
