# mysql-mcp Advanced Stress Testing: [schema-management]
...
2. **Error Path Testing**: For **every** tool, test at least **two** invalid inputs:
   ...
   - (b) An **empty parameters test** (call the tool with `{}`).
     Both must return a **structured handler error** (`{success: false, error: "..."}`) — NOT a raw MCP error frame. *(Exception: if a tool has no required parameters and defaults safely, such as `mysql_list_views` defaulting to the current database, it will legitimately return a successful response instead of an error.)*
...
4. **Wrong-Type Coercion**: For every tool with optional numeric parameters (e.g., `limit`), call the tool with `param: "abc"` (string instead of number). The tool must NOT return a raw MCP `-32602` error. *(Note: If the tool group does not contain any optional numeric parameters, you may skip this step).*
...
5. **Commit**: Stage and commit all changes — do NOT push. **CRITICAL**: Your commit message MUST explicitly include the name of this tool group prompt file (e.g. `[Testing: test-codemode-advanced-schema-management.md]`) so the history can be traced.
