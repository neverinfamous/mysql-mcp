# mysql-mcp Advanced Stress Testing: [versioning]
...
- Use `mysql_enable_versioning` on a test table, verify `_version` column and triggers exist.
- Use `mysql_disable_versioning`, verify they are cleanly removed.
- Attempt to `mysql_disable_versioning` when it's already disabled. Verify it handles gracefully.

### 2. Concurrent OCC Modifications
- Create a test table, enable versioning, insert a row.
- Read the `_version` via `mysql_check_version`.
...
### 3. Error Handling
- Call `mysql_check_version` on a non-existent table or row ID.
...
5. **Commit**: Stage and commit all changes — do NOT push. **CRITICAL**: Your commit message MUST explicitly include the name of this tool group prompt file (e.g. `[Testing: test-codemode-advanced-versioning.md]`) so the history can be traced.
