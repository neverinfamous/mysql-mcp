# mysql-mcp Advanced Stress Testing: [stats-descriptive]
...
7. Drop table `stress_stats_desc`. Verify clean removal.
...
5. **Commit**: Stage and commit all changes — do NOT push. **CRITICAL**: Your commit message MUST explicitly include the name of this tool group prompt file (e.g. `[Testing: test-codemode-advanced-stats-descriptive.md]`) so the history can be traced.
