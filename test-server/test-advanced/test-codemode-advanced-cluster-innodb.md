# mysql-mcp Advanced Stress Testing: [cluster-innodb]
...
10. `mysql_cluster_switchover()` → verify structured `{success: false}` (analysis fails with no cluster)
...
5. **Commit**: Stage and commit all changes — do NOT push. **CRITICAL**: Your commit message MUST explicitly include the name of this tool group prompt file (e.g. `[Testing: test-codemode-advanced-cluster-innodb.md]`) so the history can be traced.
