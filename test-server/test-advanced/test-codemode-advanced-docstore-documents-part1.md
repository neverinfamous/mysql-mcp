# MySQL MCP Advanced Stress Testing: [docstore-documents - part 1]
...
1. Find with empty criteria `{}` — should return all documents
2. Find with criteria matching no documents — verify empty result (not error)
3. Add document with empty object `{}` — verify insertion succeeds
4. Modify with criteria matching no documents — verify structured response
5. Remove with criteria matching no documents — verify structured response

## Category 3: Index Operations
1. Create index on JSON path for `stress_docs` collection
2. Drop collection with index — verify clean removal
## Cleanup
