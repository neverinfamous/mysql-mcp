...
- **Exporter Audit Log**: The `mysql-mcp-exporter` container reads the IDE's live audit JSONL via `AUDIT_LOG_PATH=/var/log/mysql-mcp/mcp-audit.jsonl` (mounted read-write from `../../logs`). This separates the metrics read path from the exporter's own `--audit-log` write path.
