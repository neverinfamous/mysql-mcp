...
async function run() {
    ...
    for (const node of secondaries) {
        ...
        try {
            ...
            execSync(`${dockerCmd} exec mysql-node1 mysqlsh --uri "${clusterUser}:${clusterPass}@mysql-node1:3306" --js -e "var c = dba.getCluster(); c.rejoinInstance('${clusterUser}:${clusterPass}@${node}:3306');"`, { stdio: 'ignore' });
            rejoinSuccess = true;
        } catch(e) {
            rejoinSuccess = false;
        }
        ...
        if (rejoinSuccess) {
            ...
            try {
                execSync(`${dockerCmd} exec mysql-node1 mysqlsh --uri "${clusterUser}:${clusterPass}@mysql-node1:3306" --js -e "var c = dba.getCluster(); try { c.removeInstance('${clusterUser}:${clusterPass}@${node}:3306', {force: true}); } catch(e) {}"`, { stdio: 'ignore' });
            } catch(e) {}

            console.log(`    -> Cloning instance (this will take a moment)...`);
            const clonePromise = execAsync(`${dockerCmd} exec mysql-node1 mysqlsh --uri "${clusterUser}:${clusterPass}@mysql-node1:3306" --js -e "var c = dba.getCluster(); c.addInstance('${clusterUser}:${clusterPass}@${node}:3306', {recoveryMethod: 'clone', interactive: false});"`);
            ...
            try {
                execSync(`${dockerCmd} restart mysql-router`);
            } catch(e) {}
        }
    }
