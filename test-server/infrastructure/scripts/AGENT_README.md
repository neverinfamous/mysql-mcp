...
- `toggle-super-read.ts`: Connects directly to the primary node to toggle the `super_read_only` variable on or off (e.g. `bun toggle-super-read.ts ON/OFF`). Useful for resetting state if testing leaves the node in read-only mode.
