# mysql-mcp Usability & Hallucination Test: Security
...
Use the `mysql_execute_code` tool to interact with tools in the `security` group.
...
1. If local verification passes, run `git add .` and `git commit -m "Optimize security tool usage"`.
