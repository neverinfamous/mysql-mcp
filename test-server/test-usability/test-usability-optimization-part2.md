# mysql-mcp Usability & Hallucination Test: Optimization (Part 2)

> **This test is optimized for an autonomous agent.**

This prompt instructs you to organically test the `optimization` tool group using Code Mode (`mysql_execute_code`), intentionally fuzzing the inputs to discover agent hallucinations, and permanently hardening the codebase against them.
...
- Test positional params: Try `mysql.optimization.<method>("value")` if applicable.
- Test aliases: See if intuitively named methods work (e.g. `mysql.optimization.get()`).
...
1. If local verification passes, run `git add .` and `bun .\.agents\scripts\commit.ts --msg "test(usability): Optimize optimization tool usage" --impact 0.1 --confidence 1.0 --validation passed --journal --add .`.
