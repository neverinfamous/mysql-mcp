# mysql-mcp Usability & Hallucination Test: Replication (Part 1)

> **This test is optimized for an autonomous agent.**

This prompt instructs you to organically test the `replication` tool group using Code Mode (`mysql_execute_code`), intentionally fuzzing the inputs to discover agent hallucinations, and permanently hardening the codebase against them.
...
- Guess property names: Pass `tableName` instead of `table`, `sql` instead of `query`, and `fileName` instead of `logFile` to see if they resolve correctly.
...
- Test tool names: Try calling the raw tool name directly on the group (e.g. `mysql.replication.mysql_master_status()`).
...
1. If local verification passes, run `git add .` and `bun .\.agents\scripts\commit.ts --msg "test(usability): Optimize replication tool usage" --impact 0.1 --confidence 1.0 --validation passed --journal --add .`.
