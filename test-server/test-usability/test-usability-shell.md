...
Use the `mysql_execute_code` tool to interact with tools in the `shell` group.
...
1. If local verification passes, run `git add .` and `git commit -m "Optimize shell tool usage"`.
