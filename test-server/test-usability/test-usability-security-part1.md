# mysql-mcp Usability & Hallucination Test: Security (Part 1)
...
- Test positional params: Try `mysql.security.<method>("value")` if applicable.
- Test aliases: See if intuitively named methods work (e.g. `mysql.security.get()`).
...
1. If local verification passes, run `git add .` and `bun .\.agents\scripts\commit.ts --msg "test(usability): Optimize security tool usage" --impact 0.1 --confidence 1.0 --validation passed --journal --add .`.
