# mysql-mcp Usability & Hallucination Test: Migration (Part 1)
...
1. If local verification passes, run `git add .` and `bun .\.agents\scripts\commit.ts --msg "test(usability): Optimize migration tool usage" --impact 0.1 --confidence 1.0 --validation passed --journal --add .`.
