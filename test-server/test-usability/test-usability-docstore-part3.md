# mysql-mcp Usability & Hallucination Test: Docstore (Part 3)
...
- Test aliases: See if intuitively named methods work (e.g. `mysql.docstore.get()`, `mysql.docstore.index()`).
...
1. If local verification passes, run `git add .` and `bun .\.agents\scripts\commit.ts --msg "test(usability): Optimize docstore tool usage" --impact 0.1 --confidence 1.0 --validation passed --journal --add .`.
