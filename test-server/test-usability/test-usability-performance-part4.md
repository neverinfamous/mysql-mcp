# mysql-mcp Usability & Hallucination Test: Performance (Part 4)

> **This test is optimized for an autonomous agent.**

This prompt instructs you to organically test the `performance` tool group using Code Mode (`mysql_execute_code`), intentionally fuzzing the inputs to discover agent hallucinations, and permanently hardening the codebase against them.

## 1. Fuzz Phase

Use the `mysql_execute_code` tool to interact with the following tools in the `performance` group:
...
- Test positional params: Try `mysql.performance.<method>("value")` if applicable.
- Test aliases: See if intuitively named methods work (e.g. `mysql.performance.get()`).
...
1. If local verification passes, run `git add .` and `bun .\.agents\scripts\commit.ts --msg "test(usability): Optimize performance tool usage" --impact 0.1 --confidence 1.0 --validation passed --journal --add .`.
