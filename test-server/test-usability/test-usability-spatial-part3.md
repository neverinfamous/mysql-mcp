# mysql-mcp Usability & Hallucination Test: Spatial (Part 3)
...
- Test positional params: Try `mysql.spatial.<method>("value")` if applicable.
- Test aliases: See if intuitively named methods work (e.g. `mysql.spatial.get()`).
...
1. If local verification passes, run `git add .` and `bun .\.agents\scripts\commit.ts --msg "test(usability): Optimize spatial tool usage" --impact 0.1 --confidence 1.0 --validation passed --journal --add .`.
