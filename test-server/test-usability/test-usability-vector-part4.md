# mysql-mcp Usability & Hallucination Test: Vector (Part 4)

> **This test is optimized for an autonomous agent.**

This prompt instructs you to organically test the `vector` tool group using Code Mode (`mysql_execute_code`), intentionally fuzzing the inputs to discover agent hallucinations, and permanently hardening the codebase against them.

## 1. Fuzz Phase

Use the `mysql_execute_code` tool to interact with the following tools in the `vector` group:
...
- Test positional params: Try `mysql.vector.<method>("value")` if applicable.
- Test aliases: See if intuitively named methods work (e.g. `mysql.vector.get()`).
...
1. If local verification passes, run `git add .` and `bun .\.agents\scripts\commit.ts --msg "test(usability): Optimize vector tool usage" --impact 0.1 --confidence 1.0 --validation passed --journal --add .`.
