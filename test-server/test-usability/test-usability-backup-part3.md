# mysql-mcp Usability & Hallucination Test: Backup (Part 3)

> **This test is optimized for an autonomous agent.**

This prompt instructs you to organically test the `backup` tool group using Code Mode (`mysql_execute_code`), intentionally fuzzing the inputs to discover agent hallucinations, and permanently hardening the codebase against them.

## 1. Fuzz Phase

Use the `mysql_execute_code` tool to interact with the following tools in the `backup` group:
...
- Test positional params: Try `mysql.backup.<method>("value")` if applicable.
- Test aliases: See if intuitively named methods work (e.g. `mysql.backup.get()`).
...
1. If local verification passes, run `git add .` and `bun .\.agents\scripts\commit.ts --msg "test(usability): Optimize backup tool usage" --impact 0.1 --confidence 1.0 --validation passed --journal --add .`.
