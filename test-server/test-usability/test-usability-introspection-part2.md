...
1. **ONLY if you made modifications** (code or prompt):
   - Run `git add .` and `bun .\.agents\scripts\commit.ts --msg "test(usability): Optimize introspection tool usage" --impact 0.1 --confidence 1.0 --validation passed --journal --add .`.
