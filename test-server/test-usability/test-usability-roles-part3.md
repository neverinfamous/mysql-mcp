# mysql-mcp Usability & Hallucination Test: Roles (Part 3)

> **This test is optimized for an autonomous agent.**

This prompt instructs you to organically test the `roles` tool group using Code Mode (`mysql_execute_code`), intentionally fuzzing the inputs to discover agent hallucinations, and permanently hardening the codebase against them.

## 1. Fuzz Phase

Use the `mysql_execute_code` tool to interact with the following tools in the `roles` group:
- `mysql_role_revoke`
- `mysql_user_roles`
...
- Test positional params: Try `mysql.roles.<method>("value")` if applicable.
- Test aliases: See if intuitively named methods work (e.g. `mysql.roles.get()`, `mysql.roles.getUserRoles()`, `mysql.roles.revokeRole()`).
...
1. If local verification passes, run `git add .` and `bun .\.agents\scripts\commit.ts --msg "test(usability): Optimize roles tool usage" --impact 0.1 --confidence 1.0 --validation passed --journal --add .`.
