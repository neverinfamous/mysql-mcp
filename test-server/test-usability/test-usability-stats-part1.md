# mysql-mcp Usability & Hallucination Test: Stats (Part 1)
...
- Guess property names: Pass `tableName` instead of `table`, `sql` instead of `query` to see if they resolve correctly. Try `tbl`, `table_name`, `c1`, `c2`, `p`, etc.
...
1. If local verification passes, run `git add .` and `bun .\.agents\scripts\commit.ts --msg "test(usability): Optimize stats tool usage" --impact 0.1 --confidence 1.0 --validation passed --journal --add .`.
