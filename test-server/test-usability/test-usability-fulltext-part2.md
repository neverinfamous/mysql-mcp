# mysql-mcp Usability & Hallucination Test: Fulltext (Part 2)

> **This test is optimized for an autonomous agent.**

This prompt instructs you to organically test the `fulltext` tool group using Code Mode (`mysql_execute_code`), intentionally fuzzing the inputs to discover agent hallucinations, and permanently hardening the codebase against them.
...
- Test positional params: Try `mysql.fulltext.<method>("value")` if applicable.
- Test aliases: See if intuitively named methods work (e.g. `mysql.fulltext.get()`).
...
1. **ONLY if you made modifications** (code or prompt):
   - Run `git add .` and `bun .\.agents\scripts\commit.ts --msg "test(usability): Optimize fulltext tool usage" --impact 0.1 --confidence 1.0 --validation passed --journal --add .`.
