...
- Guess property names: Pass `script`, `javascript`, `execute`, or `eval` instead of `code` to see if they resolve correctly.
...
1. If local verification passes, run `git add .` and `bun .\.agents\scripts\commit.ts --msg "test(usability): Optimize codemode tool usage" --impact 0.1 --confidence 1.0 --validation passed --journal --add .`.
