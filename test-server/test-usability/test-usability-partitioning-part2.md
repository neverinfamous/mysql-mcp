# mysql-mcp Usability & Hallucination Test: Partitioning (Part 2)

> **This test is optimized for an autonomous agent.**

This prompt instructs you to organically test the `partitioning` tool group using Code Mode (`mysql_execute_code`), intentionally fuzzing the inputs to discover agent hallucinations, and permanently hardening the codebase against them.
...
- Test positional params: Try `mysql.partitioning.<method>("value")` if applicable.
- Test aliases: See if intuitively named methods work (e.g. `mysql.partitioning.get()`).
...
1. If local verification passes, run `git add .` and `bun .\.agents\scripts\commit.ts --msg "test(usability): Optimize partitioning tool usage" --impact 0.1 --confidence 1.0 --validation passed --journal --add .`.
