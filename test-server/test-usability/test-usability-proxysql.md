...
Use the `mysql_execute_code` tool to interact with tools in the `proxysql` group.
...
- Test positional params: Try `mysql.proxysql.<method>("value")` if applicable.
- Test aliases: See if intuitively named methods work (e.g. `mysql.proxysql.get()`).
...
1. If local verification passes, run `git add .` and `git commit -m "Optimize proxysql tool usage"`.
