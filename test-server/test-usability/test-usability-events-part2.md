# mysql-mcp Usability & Hallucination Test: Events (Part 2)

> **This test is optimized for an autonomous agent.**

This prompt instructs you to organically test the `events` tool group using Code Mode (`mysql_execute_code`), intentionally fuzzing the inputs to discover agent hallucinations, and permanently hardening the codebase against them.
...
- Guess property names: Pass `eventName` instead of `event`, `name` instead of `eventName`, or `schema` instead of `database` to see if they resolve correctly.
...
1. If local verification passes, run `git add .` and `bun .\.agents\scripts\commit.ts --msg "test(usability): Optimize events tool usage" --impact 0.1 --confidence 1.0 --validation passed --journal --add .`.
