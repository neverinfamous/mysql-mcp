# mysql-mcp Usability & Hallucination Test: Sysschema
...
Use the `mysql_execute_code` tool to interact with tools in the `sysschema` group.
...
1. If local verification passes, run `git add .` and `git commit -m "Optimize sysschema tool usage"`.
