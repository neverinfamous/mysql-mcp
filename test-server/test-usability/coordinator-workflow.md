...
7. **Coordinator Progress Reporting**:
   - The Coordinator MUST respond to the user with ONLY this exact format as each test proceeds: This is test X out of Y. X fixes applied [Y Prompt / Z Code] [W Graceful Fails]: <concise description>. The Coordinator MUST explicitly tell the user how many prompt fixes were made, code fixes were made, and graceful degradations were experienced. Do not wrap the message in quotes or add preamble.
