...
Execute usability tests in `test-server/test-usability/`. Fuzz tools to trigger agent hallucinations. Harden the codebase using the `/mcp:mysql-mcp:mysql_mcp_heal` prompt.
...
3. **Local Verification (NO PAUSING)**:
   ...
   - DO NOT perform live server verification. DO NOT wait for a server restart. DO NOT pause or send a message asking the user to refresh the server. The restart script `bun .\.agents\scripts\restart-mcp.ts <server-name>` handles daemon resurrection dynamically.
...
7. **Coordinator Progress Reporting**:
   - The Coordinator MUST respond to the user with ONLY this exact format as each test proceeds: "Test X (<test name>) out of Y: A Prompt Fixes / B Code Fixes / C Infrastructure Absent" (e.g., "Test 32 (Spatial queries part 1) out of 56: 1 Prompt Fixes / 0 Code Fixes / 0 Infrastructure Absent")
