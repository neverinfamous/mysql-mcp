# MySQL MCP Direct Usability Testing Suite
...
> **Validation Strictness Note**: If you or a subagent modifies the codebase, run ONLY `pnpm run lint`, `pnpm run typecheck`, and `pnpm run build`. Do NOT run `pnpm run test` or `pnpm run check` to save context window tokens. Do NOT run validation for documentation-only changes. The coordinator will handle fixing broken tests at the end of the test suite.
