...
2. `mysql.cluster.grStatus()` → group replication status
3. `mysql.cluster.grMembers()` → group members status
4. `mysql.cluster.grPrimary()` → current primary details
5. `mysql.cluster.grTransactions()` → transaction stats
...
13. `mysql.cluster.switchover()` → switchover analysis
...
5. **Commit**: Stage and commit all changes — do NOT push. **CRITICAL**: Your commit message MUST explicitly include the name of this tool group prompt file (e.g. `[Testing: test-codemode-cluster-innodb.md]`) so the history can be traced.
