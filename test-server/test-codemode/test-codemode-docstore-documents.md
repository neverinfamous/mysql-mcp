# mysql-mcp Code Mode Testing: [docstore-documents]
...
3. `mysql.docstore.add({collection: "test_documents", documents: [{name: "test"}]})` → IDs
4. `mysql.docstore.modify({collection: "test_documents", condition: "$.name = 'test'", set: {"$.status": "active"}})` → affected count
5. `mysql.docstore.remove({collection: "test_documents", condition: "$.name = 'test'"})` → affected count
...
5. **Commit**: Stage and commit all changes — do NOT push. **CRITICAL**: Your commit message MUST explicitly include the name of this tool group prompt file (e.g. `[Testing: test-codemode-docstore-documents.md]`) so the history can be traced.
