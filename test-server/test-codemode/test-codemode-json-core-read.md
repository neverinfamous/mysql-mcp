# mysql-mcp Code Mode Testing: [json-core-read]
...
3. `mysql.json.contains({table: "test_json_docs", column: "doc", candidate: '{"status":"active"}'})` → matches
...
5. **Commit**: Stage and commit all changes — do NOT push. **CRITICAL**: Your commit message MUST explicitly include the name of this tool group prompt file (e.g. `[Testing: test-codemode-json-core-read.md]`) so the history can be traced.
