# mysql-mcp Code Mode Testing: [vector-management]
...
5. **Commit**: Stage and commit all changes — do NOT push. **CRITICAL**: Your commit message MUST explicitly include the name of this tool group prompt file (e.g. `[Testing: test-codemode-vector-management.md]`) so the history can be traced.
