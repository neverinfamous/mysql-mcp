# mysql-mcp Code Mode Testing: [performance-analysis-queries]
...
4. `mysql.performance.queryStats({limit: 3})` → top queries

**Domain error paths (🔴):**
5. 🔴 `mysql.performance.explain({query: "SELEKT * FROM test_products"})` → `{success: false}` syntax error

**Zod validation error paths (🔴):**
6. 🔴 `mysql.performance.explain({})` → `{success: false, error: "Validation error: ..."}`
...
5. **Commit**: Stage and commit all changes — do NOT push. **CRITICAL**: Your commit message MUST explicitly include the name of this tool group prompt file (e.g. `[Testing: test-codemode-performance-analysis-queries.md]`) so the history can be traced.
