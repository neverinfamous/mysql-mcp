# MySQL MCP Code Mode Testing: [json-enhanced-part2]
...
1. `mysql.json.normalize({...})` → happy path
2. `mysql.json.stats({...})` → happy path
3. `mysql.json.indexSuggest({...})` → happy path

**Domain error paths (🔴):**

4. 🔴 `mysql.json.normalize({...})` → domain error
5. 🔴 `mysql.json.stats({...})` → domain error
6. 🔴 `mysql.json.indexSuggest({...})` → domain error

**Zod validation error paths (🔴):**

7. 🔴 `mysql.json.normalize({})` → validation error
8. 🔴 `mysql.json.stats({})` → validation error
9. 🔴 `mysql.json.indexSuggest({})` → validation error
