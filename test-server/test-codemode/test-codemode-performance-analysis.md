# mysql-mcp Code Mode Testing: [performance-analysis]
...
5. **Commit**: Stage and commit all changes — do NOT push. **CRITICAL**: Your commit message MUST explicitly include the name of this tool group prompt file (e.g. `[Testing: test-codemode-performance-analysis.md]`) so the history can be traced.
