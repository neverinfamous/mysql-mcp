...
9. 🔴 `mysql.migration.apply({version: "1.0.2", description: "duplicate", query: "..."})` → `{success: false}`
...
5. **Commit**: Stage and commit all changes — do NOT push. **CRITICAL**: Your commit message MUST explicitly include the name of this tool group prompt file (e.g. `[Testing: test-codemode-migration.md]`) so the history can be traced.
