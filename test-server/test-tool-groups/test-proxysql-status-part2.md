# mysql-mcp Tool Group Testing: PART 2 [proxysql-status]
...
5. **Commit**: Stage and commit all changes — do NOT push. **CRITICAL**: Your commit message MUST explicitly include the name of this tool group prompt file (e.g. `[Testing: test-proxysql-status.md]`) so the history can be traced.
