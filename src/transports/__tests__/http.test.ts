...
describe('HttpTransport', () => {
    ...
    describe('setCorsHeaders()', () => {
        it('should not set CORS headers when origin not in allowed list', () => {
            ...
            (transportWithCors as unknown as { setCorsHeaders: (req: IncomingMessage, res: ServerResponse) => void })
    ...
    describe('handleHealthCheck()', () => {
        it('should return healthy status', () => {
            ...
            (transport as unknown as { handleHealthCheck: (res: ServerResponse) => void })
    ...
    describe('handleProtectedResourceMetadata()', () => {
        it('should return 404 when OAuth not configured', () => {
            ...
            (transport as unknown as { handleProtectedResourceMetadata: (res: ServerResponse) => void })
        ...
        it('should return metadata when OAuth configured', () => {
            ...
            (transportWithOAuth as unknown as { handleProtectedResourceMetadata: (res: ServerResponse) => void })
                .handleProtectedResourceMetadata(mockRes);
    ...
    describe('setSecurityHeaders()', () => {
        it('should set X-Content-Type-Options header', () => {
            ...
            (transport as unknown as { setSecurityHeaders: (res: ServerResponse) => void })
        ...
        it('should set Cache-Control header', () => {
            ...
            expect(mockRes.setHeader).toHaveBeenCalledWith('Cache-Control', 'no-store, no-cache, must-revalidate');
        });
...
describe('handleRequest()', () => {
    ...
    it('should handle OPTIONS preflight request', async () => {
        ...
        await (transport as unknown as { handleRequest: (req: IncomingMessage, res: ServerResponse) => Promise<void> })
    ...
    it('should reject unauthenticated requests when OAuth is configured', async () => {
        const mockTokenValidator = {
            validate: vi.fn().mockResolvedValue({ valid: false, error: 'Token missing' })
        };
        ...
        await (transportWithOAuth as unknown as { handleRequest: (req: IncomingMessage, res: ServerResponse) => Promise<void> })
            .handleRequest(mockReq, mockRes);
...
describe('SSE Support', () => {
    it('should route to SSE endpoint', async () => {
        ...
        const transportWithCallback = createHttpTransport({ port: 3000 }, onConnect);
        const { StreamableHTTPServerTransport } = await import('@modelcontextprotocol/sdk/server/streamableHttp.js');

        await (transportWithCallback as unknown as { handleRequest: (req: IncomingMessage, res: ServerResponse) => Promise<void> })
            .handleRequest(mockReq, mockRes);

        expect(StreamableHTTPServerTransport).toHaveBeenCalled();
        // Verify start called (mocked)
        const mockTransportInstance = (StreamableHTTPServerTransport as unknown as ReturnType<typeof vi.fn>).mock.results[0].value;
    ...
    it('should return 400 for messages without active connection', async () => {
        ...
        const endCall = (mockRes.end as ReturnType<typeof vi.fn>).mock.calls[0][0];
        expect(JSON.parse(endCall as string)).toHaveProperty('error', 'No active connection');
    });
});
