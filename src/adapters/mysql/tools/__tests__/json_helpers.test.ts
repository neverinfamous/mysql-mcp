...
describe('getJsonHelperTools', () => {
    ...
    beforeEach(() => {
        vi.clearAllMocks();
        tools = getJsonHelperTools(createMockMySQLAdapter() as unknown as MySQLAdapter);
    });
...
describe('JSON Helper Handler Execution', () => {
    ...
    describe('mysql_json_validate', () => {
        it('should validate JSON value', async () => {
            ...
            const result = await tool.handler({ value: '{"name":"test"}' }, mockContext);
    ...
    describe('mysql_json_update', () => {
        ...
        it('should return failure when no rows updated', async () => {
            mockAdapter.executeWriteQuery.mockResolvedValue({ rows: [], rowsAffected: 0, executionTimeMs: 5 });
