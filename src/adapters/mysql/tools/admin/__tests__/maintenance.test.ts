...
describe('Admin Maintenance Tools', () => {
    ...
    describe('createOptimizeTableTool', () => {
        it('should create tool with correct definition', () => {
            const tool = createOptimizeTableTool(mockAdapter as unknown as MySQLAdapter);
        ...
        it('should execute OPTIMIZE TABLE for single table', async () => {
            mockAdapter.executeQuery.mockResolvedValue(
                createMockQueryResult([{ Table: 'users', Op: 'optimize', Msg_type: 'status', Msg_text: 'OK' }])
            );

            const tool = createOptimizeTableTool(mockAdapter as unknown as MySQLAdapter);
            const result = await tool.handler({ tables: ['users'] }, mockContext);

            expect(mockAdapter.executeQuery).toHaveBeenCalledWith('OPTIMIZE TABLE `users`');
        ...
        it('should execute OPTIMIZE TABLE for multiple tables', async () => {
            ...
            const tool = createOptimizeTableTool(mockAdapter as unknown as MySQLAdapter);
            await tool.handler({ tables: ['users', 'orders', 'products'] }, mockContext);

            expect(mockAdapter.executeQuery).toHaveBeenCalledWith('OPTIMIZE TABLE `users`, `orders`, `products`');
        });
    ...
    describe('createAnalyzeTableTool', () => {
        it('should create tool with correct definition', () => {
            const tool = createAnalyzeTableTool(mockAdapter as unknown as MySQLAdapter);
        ...
        it('should execute ANALYZE TABLE for single table', async () => {
            mockAdapter.executeQuery.mockResolvedValue(
                createMockQueryResult([{ Table: 'products', Op: 'analyze', Msg_type: 'status', Msg_text: 'OK' }])
            );

            const tool = createAnalyzeTableTool(mockAdapter as unknown as MySQLAdapter);
            const result = await tool.handler({ tables: ['products'] }, mockContext);

            expect(mockAdapter.executeQuery).toHaveBeenCalledWith('ANALYZE TABLE `products`');
            expect(result).toHaveProperty('results');
        });
        ...
        it('should execute ANALYZE TABLE for multiple tables', async () => {
            ...
            expect(mockAdapter.executeQuery).toHaveBeenCalledWith('ANALYZE TABLE `table1`, `table2`');
        });
    });
    ...
    describe('createCheckTableTool', () => {
        ...
        it('should execute CHECK TABLE without option', async () => {
            mockAdapter.rawQuery.mockResolvedValue(
                createMockQueryResult([{ Table: 'users', Op: 'check', Msg_type: 'status', Msg_text: 'OK' }])
            );
        ...
        it('should execute CHECK TABLE with EXTENDED option', async () => {
            ...
            const tool = createCheckTableTool(mockAdapter as unknown as MySQLAdapter);
            await tool.handler({ tables: ['users'], option: 'EXTENDED' }, mockContext);

            expect(mockAdapter.rawQuery).toHaveBeenCalledWith('CHECK TABLE `users` EXTENDED');
        });
        ...
        it('should execute CHECK TABLE with QUICK option', async () => {
            ...
            expect(mockAdapter.rawQuery).toHaveBeenCalledWith('CHECK TABLE `orders` QUICK');
        });
        ...
        it('should execute CHECK TABLE for multiple tables with option', async () => {
            ...
            const tool = createCheckTableTool(mockAdapter as unknown as MySQLAdapter);
            await tool.handler({ tables: ['t1', 't2'], option: 'MEDIUM' }, mockContext);

            expect(mockAdapter.rawQuery).toHaveBeenCalledWith('CHECK TABLE `t1`, `t2` MEDIUM');
        });
        ...
        it('should handle empty result rows', async () => {
            ...
            const tool = createCheckTableTool(mockAdapter as unknown as MySQLAdapter);
            const result = await tool.handler({ tables: ['users'] }, mockContext) as { results: unknown[]; rowCount: number };
        ...
        it('should handle missing rows property', async () => {
            mockAdapter.rawQuery.mockResolvedValue({ rows: undefined, rowsAffected: 0, executionTimeMs: 5 });
    ...
    describe('createRepairTableTool', () => {
        it('should create tool with correct definition', () => {
            const tool = createRepairTableTool(mockAdapter as unknown as MySQLAdapter);
        ...
        it('should execute REPAIR TABLE without QUICK option', async () => {
            mockAdapter.executeQuery.mockResolvedValue(
                createMockQueryResult([{ Table: 'myisam_table', Op: 'repair', Msg_type: 'status', Msg_text: 'OK' }])
            );

            const tool = createRepairTableTool(mockAdapter as unknown as MySQLAdapter);
            const result = await tool.handler({ tables: ['myisam_table'] }, mockContext);

            expect(mockAdapter.executeQuery).toHaveBeenCalledWith('REPAIR TABLE `myisam_table`');
            expect(result).toHaveProperty('results');
        });
        ...
        it('should execute REPAIR TABLE with QUICK option', async () => {
            ...
            expect(mockAdapter.executeQuery).toHaveBeenCalledWith('REPAIR TABLE `old_table` QUICK');
        });
        ...
        it('should execute REPAIR TABLE for multiple tables', async () => {
            ...
            expect(mockAdapter.executeQuery).toHaveBeenCalledWith('REPAIR TABLE `t1`, `t2`');
        });
    });

    describe('createFlushTablesTool', () => {
        it('should create tool with correct definition', () => {
            const tool = createFlushTablesTool(mockAdapter as unknown as MySQLAdapter);
        ...
        it('should execute FLUSH TABLES for specific table', async () => {
            ...
            expect(mockAdapter.executeQuery).toHaveBeenCalledWith('FLUSH TABLES `users`');
            expect(result).toEqual({ success: true });
        });
        ...
        it('should execute FLUSH TABLES for multiple specific tables', async () => {
            ...
            expect(mockAdapter.executeQuery).toHaveBeenCalledWith('FLUSH TABLES `users`, `orders`, `products`');
        });
    });
    ...
    describe('createKillQueryTool', () => {
        ...
        it('should execute KILL QUERY when connection is false', async () => {
            ...
            const tool = createKillQueryTool(mockAdapter as unknown as MySQLAdapter);
            const result = await tool.handler({ processId: 999, connection: false }, mockContext);
        ...
        it('should execute KILL CONNECTION when connection is true', async () => {
            ...
            const tool = createKillQueryTool(mockAdapter as unknown as MySQLAdapter);
            const result = await tool.handler({ processId: 54321, connection: true }, mockContext);

            expect(mockAdapter.executeQuery).toHaveBeenCalledWith('KILL CONNECTION 54321');
            expect(result).toEqual({ success: true, killed: 54321, type: 'CONNECTION' });
        });
        ...
        it('should handle various process ID formats', async () => {
            ...
            expect(mockAdapter.executeQuery).toHaveBeenLastCalledWith('KILL QUERY 2147483647');
        });
    });
});
