...
describe('Admin Monitoring Tools', () => {
    ...
    describe('createShowProcesslistTool', () => {
        it('should create tool with correct definition', () => {
            const tool = createShowProcesslistTool(mockAdapter as unknown as MySQLAdapter);
        ...
        it('should show processlist', async () => {
            mockAdapter.executeQuery.mockResolvedValue(createMockQueryResult([
                { Id: 1, User: 'root', Host: 'localhost', db: 'test', Command: 'Query', Time: 0, State: 'executing', Info: 'SHOW PROCESSLIST' }
            ]));

            const tool = createShowProcesslistTool(mockAdapter as unknown as MySQLAdapter);
            const result = await tool.handler({ full: true }, mockContext);

            expect(mockAdapter.executeQuery).toHaveBeenCalledWith('SHOW FULL PROCESSLIST');
    ...
    describe('createShowStatusTool', () => {
        ...
        it('should transform results to key-value pairs', async () => {
            ...
            const tool = createShowStatusTool(mockAdapter as unknown as MySQLAdapter);
            const result = await tool.handler({}, mockContext) as { status: Record<string, string> };
    ...
    describe('createShowVariablesTool', () => {
        it('should create tool with correct definition', () => {
            const tool = createShowVariablesTool(mockAdapter as unknown as MySQLAdapter);
        ...
        it('should show global variables without filter', async () => {
            ...
            expect(mockAdapter.rawQuery).toHaveBeenCalledWith('SHOW GLOBAL VARIABLES');
        ...
        it('should transform results to key-value pairs', async () => {
            ...
            const tool = createShowVariablesTool(mockAdapter as unknown as MySQLAdapter);
            const result = await tool.handler({}, mockContext) as { variables: Record<string, string> };
    ...
    describe('createInnodbStatusTool', () => {
        it('should create tool with correct definition', () => {
            const tool = createInnodbStatusTool(mockAdapter as unknown as MySQLAdapter);
        ...
        it('should execute SHOW ENGINE INNODB STATUS', async () => {
            mockAdapter.executeQuery.mockResolvedValue(createMockQueryResult([
                { Type: 'InnoDB', Name: '', Status: 'BACKGROUND THREAD\n...\nEND OF INNODB MONITOR OUTPUT' }
            ]));

            const tool = createInnodbStatusTool(mockAdapter as unknown as MySQLAdapter);
            const result = await tool.handler({}, mockContext);

            expect(mockAdapter.executeQuery).toHaveBeenCalledWith('SHOW ENGINE INNODB STATUS');
            expect(result).toHaveProperty('status');
        });
    ...
    describe('createReplicationStatusTool', () => {
        it('should create tool with correct definition', () => {
            const tool = createReplicationStatusTool(mockAdapter as unknown as MySQLAdapter);
        ...
        it('should handle no replication configured', async () => {
            ...
            const tool = createReplicationStatusTool(mockAdapter as unknown as MySQLAdapter);
            const result = await tool.handler({}, mockContext) as { status: unknown[] };
        ...
        it('should fallback to SHOW SLAVE STATUS on error', async () => {
            ...
            mockAdapter.executeQuery
                ...
                .mockResolvedValueOnce(createMockQueryResult([{ Master_Host: 'master.db', Slave_IO_Running: 'Yes' }]));
    ...
    describe('createPoolStatsTool', () => {
        ...
        it('should handle pool error', async () => {
            const mockPool = {
                getStats: vi.fn().mockImplementation(() => { throw new Error('Pool error'); })
            };
            mockAdapter.getPool = vi.fn().mockReturnValue(mockPool);
    ...
    describe('createServerHealthTool', () => {
        it('should create tool with correct definition', () => {
            const tool = createServerHealthTool(mockAdapter as unknown as MySQLAdapter);
        ...
        it('should execute health check query', async () => {
            mockAdapter.executeQuery.mockResolvedValue(createMockQueryResult([{ result: 1 }]));
        ...
        it('should query server status metrics', async () => {
            mockAdapter.executeQuery
                .mockResolvedValueOnce(createMockQueryResult([{ Variable_name: 'Uptime', Value: '86400' }]))
                .mockResolvedValueOnce(createMockQueryResult([{ Variable_name: 'Threads_connected', Value: '10' }]))
                .mockResolvedValueOnce(createMockQueryResult([{ Variable_name: 'Questions', Value: '12345' }]));
        ...
        it('should handle partial status variables', async () => {
            mockAdapter.executeQuery
                .mockResolvedValueOnce(createMockQueryResult([{ Variable_name: 'Uptime', Value: '3600' }]))
                .mockResolvedValueOnce(createMockQueryResult([]))
                .mockResolvedValueOnce(createMockQueryResult([]));
