...
function validateJsonString(value: unknown): string {
    if (typeof value === 'string') {
        try {
            ...
            throw new Error(`Invalid JSON value. To store a string/text, it must be quoted (e.g., '"my_string"'). Value provided: ${value}`);
        }
    }
...
export function createJsonArrayAppendTool(adapter: MySQLAdapter): ToolDefinition {
    const schema = z.object({
        table: z.string(),
        column: z.string(),
        path: z.string(),
