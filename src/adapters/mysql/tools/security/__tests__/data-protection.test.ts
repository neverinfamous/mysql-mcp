...
describe('Security Data Protection Tools', () => {
    ...
    describe('createSecurityMaskDataTool', () => {
        it('should mask email addresses', async () => {
            const tool = createSecurityMaskDataTool(mockAdapter as unknown as MySQLAdapter);
            const result = await tool.handler({
                value: 'john.doe@example.com',
                type: 'email'
            }, mockContext) as { masked: string };
    ...
    describe('createSecurityUserPrivilegesTool', () => {
        it('should get user privileges and roles', async () => {
            ...
            mockAdapter.executeQuery.mockResolvedValueOnce(createMockQueryResult([
                { User: 'john', Host: 'localhost', authPlugin: 'native', accountLocked: 'N', passwordExpired: 'N' }
            ]));

            // Mock permissions query for john
            mockAdapter.executeQuery.mockResolvedValueOnce(createMockQueryResult([
                { 'Grants for john@localhost': 'GRANT ALL PRIVILEGES ON *.* TO \'john\'@\'localhost\'' }
            ]));

            // Mock roles query
            mockAdapter.executeQuery.mockResolvedValueOnce(createMockQueryResult([
                { FROM_USER: 'admin_role', FROM_HOST: '%' }
            ]));


            const tool = createSecurityUserPrivilegesTool(mockAdapter as unknown as MySQLAdapter);
            const result = await tool.handler({
                user: 'john'
            }, mockContext) as { users: any[] };
    ...
    describe('createSecuritySensitiveTablesTool', () => {
        it('should identify sensitive tables', async () => {
            mockAdapter.executeQuery.mockResolvedValue(createMockQueryResult([
                ...
                { tableName: 'payments', columnName: 'credit_card', dataType: 'varchar' }
            ]));

            const tool = createSecuritySensitiveTablesTool(mockAdapter as unknown as MySQLAdapter);
            const result = await tool.handler({
                schema: 'test_db'
            }, mockContext) as { sensitiveTables: any[], totalSensitiveColumns: number };
