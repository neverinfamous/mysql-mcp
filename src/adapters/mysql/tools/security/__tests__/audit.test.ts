...
describe('Security Audit Tools', () => {
    ...
    describe('createSecurityAuditTool', () => {
        it('should query mysql.audit_log if available', async () => {
            ...
            mockAdapter.executeQuery.mockResolvedValueOnce(createMockQueryResult([
                { user: 'test', event_type: 'LOGIN' }
            ]));

            const tool = createSecurityAuditTool(mockAdapter as unknown as MySQLAdapter);
            const result = await tool.handler({
                limit: 10,
                user: 'test'
            }, mockContext) as { source: string, events: any[] };
        ...
        it('should fallback to performance_schema if audit_log is missing', async () => {
            ...
            const result = await tool.handler({
                limit: 10
            }, mockContext) as { source: string };

            expect(result.source).toBe('performance_schema');

            const queryCall = mockAdapter.executeQuery.mock.calls[1][0] as string;
            expect(queryCall).toContain('FROM performance_schema.events_statements_history');
            expect(queryCall).toContain('FROM performance_schema.events_statements_history');
        });
        ...
        it('should handle error when checking audit log', async () => {
            ...
            const tool = createSecurityAuditTool(mockAdapter as unknown as MySQLAdapter);
            const result = await tool.handler({}, mockContext) as { available: boolean };
    ...
    describe('createSecurityFirewallStatusTool', () => {
        it('should check firewall status when installed', async () => {
            ...
            const tool = createSecurityFirewallStatusTool(mockAdapter as unknown as MySQLAdapter);
            const result = await tool.handler({}, mockContext) as { installed: boolean, configuration: any };
        ...
        it('should report not installed if plugin missing', async () => {
            ...
            const tool = createSecurityFirewallStatusTool(mockAdapter as unknown as MySQLAdapter);
            const result = await tool.handler({}, mockContext) as { installed: boolean };
    ...
    describe('createSecurityFirewallRulesTool', () => {
        it('should list firewall rules', async () => {
            // Mock users query
            mockAdapter.executeQuery.mockResolvedValueOnce(createMockQueryResult([
                { USERHOST: 'user@%', MODE: 'PROTECTING' }
            ]));

            // Mock rules query
            mockAdapter.executeQuery.mockResolvedValueOnce(createMockQueryResult([
                { USERHOST: 'user@%', RULE: 'SELECT *' }
            ]));

            const tool = createSecurityFirewallRulesTool(mockAdapter as unknown as MySQLAdapter);
            const result = await tool.handler({
                user: 'user'
            }, mockContext) as { users: any[], rules: any[] };
