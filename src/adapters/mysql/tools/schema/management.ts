...
const ListSchemasSchema = z.object({
    pattern: z.string().optional().describe('Filter pattern (LIKE syntax, e.g. "app_%")')
});
...
export function createListSchemasTool(adapter: MySQLAdapter): ToolDefinition {
    return {
        ...
        description: 'List all databases/schemas with metadata including charset and collation.',
        group: 'schema',
...
export function createCreateSchemaTool(adapter: MySQLAdapter): ToolDefinition {
    return {
        ...
        description: 'Create a new database/schema with specified charset and collation.',
        group: 'schema',
        ...
        handler: async (params: unknown, _context: RequestContext) => {
            const { name, charset, collation, ifNotExists } = CreateSchemaSchema.parse(params);
...
export function createDropSchemaTool(adapter: MySQLAdapter): ToolDefinition {
    return {
        ...
        description: 'Drop a database/schema. WARNING: This permanently deletes all data.',
        group: 'schema',
