...
describe('Schema Constraint Tools', () => {
    ...
    describe('mysql_list_constraints', () => {
        it('should query INFORMATION_SCHEMA for constraints', async () => {
            ...
            const tool = createListConstraintsTool(mockAdapter as unknown as MySQLAdapter);
        ...
        it('should handle special characters in schema-qualified names', async () => {
            mockAdapter.executeQuery.mockResolvedValue(createMockQueryResult([
                { name: 'fk_orders', type: 'FOREIGN KEY', referencedTable: 'customers' }
            ]));

            const tool = createListConstraintsTool(mockAdapter as unknown as MySQLAdapter);
            const result = await tool.handler({ table: 'test_db.order_items' }, mockContext);
