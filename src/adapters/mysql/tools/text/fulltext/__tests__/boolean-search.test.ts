...
import { createFulltextBooleanTool } from "../boolean-search.js";
import type { MySQLAdapter } from "../../../../mysql-adapter/index.js";

describe("createFulltextBooleanTool", () => {
  let mockAdapter: ReturnType<typeof vi.mocked<MySQLAdapter>>;
  let tool: ReturnType<typeof createFulltextBooleanTool>;
  ...
  it("should perform boolean mode search", async () => {
    ...
    expect(mockAdapter.executeReadQuery).toHaveBeenCalledWith(
      "WITH cte AS (SELECT *, MATCH(`title`) AGAINST(? IN BOOLEAN MODE) as relevance FROM `articles` WHERE MATCH(`title`) AGAINST(? IN BOOLEAN MODE)) SELECT * FROM cte ORDER BY relevance DESC LIMIT 5",
