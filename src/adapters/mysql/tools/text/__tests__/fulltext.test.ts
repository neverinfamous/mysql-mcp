...
describe('Text Fulltext Tools', () => {
    ...
    describe('createFulltextCreateTool', () => {
        it('should create tool with correct definition', () => {
            const tool = createFulltextCreateTool(mockAdapter as unknown as MySQLAdapter);
        ...
        it('should create fulltext index', async () => {
            ...
            const result = await tool.handler({
                ...
                indexName: 'ft_idx'
            }, mockContext) as { indexName: string };

            expect(mockAdapter.executeQuery).toHaveBeenCalled();
            const call = mockAdapter.executeQuery.mock.calls[0][0] as string;
            expect(call).toContain('CREATE FULLTEXT INDEX `ft_idx` ON `articles` (`title`, `content`)');
        ...
        it('should generate index name if not provided', async () => {
            ...
            expect(mockAdapter.executeQuery).toHaveBeenCalled();
            const call = mockAdapter.executeQuery.mock.calls[0][0] as string;
            expect(call).toContain('CREATE FULLTEXT INDEX `ft_articles_title_content`');
    ...
    describe('createFulltextSearchTool', () => {
        it('should create tool with correct definition', () => {
            const tool = createFulltextSearchTool(mockAdapter as unknown as MySQLAdapter);
        ...
        it('should search in natural language mode by default', async () => {
            ...
            expect(call).toContain('MATCH(`title`, `content`) AGAINST(? IN NATURAL LANGUAGE MODE)');
        });
    ...
    describe('createFulltextBooleanTool', () => {
        it('should create tool with correct definition', () => {
            const tool = createFulltextBooleanTool(mockAdapter as unknown as MySQLAdapter);
    ...
    describe('createFulltextExpandTool', () => {
        it('should create tool with correct definition', () => {
            const tool = createFulltextExpandTool(mockAdapter as unknown as MySQLAdapter);
