...
describe('Text Processing Tools', () => {
    ...
    describe('createRegexpMatchTool', () => {
        it('should perform regex match query', async () => {
            ...
            const tool = createRegexpMatchTool(mockAdapter as unknown as MySQLAdapter);
    ...
    describe('createSubstringTool', () => {
        it('should extract substring with length', async () => {
            mockAdapter.executeReadQuery.mockResolvedValue(createMockQueryResult([{ substring_value: 'abc' }]));
    ...
    describe('createConcatTool', () => {
        it('should concatenate columns', async () => {
            mockAdapter.executeReadQuery.mockResolvedValue(createMockQueryResult([{ full_name: 'John Doe' }]));
            ...
            const call = mockAdapter.executeReadQuery.mock.calls[0][0] as string;
            expect(call).toContain("CONCAT_WS(?, `first_name`, `last_name`) as `full_name`");
        });
    ...
    describe('createCollationConvertTool', () => {
        it('should convert charset', async () => {
            ...
            const tool = createCollationConvertTool(mockAdapter as unknown as MySQLAdapter);
            await tool.handler({
                table: 'legacy',
                column: 'text_col',
                charset: 'utf8mb4'
        ...
        it('should convert charset and collation', async () => {
            ...
            const call = mockAdapter.executeReadQuery.mock.calls[0][0] as string;
            expect(call).toContain('CONVERT(`text_col` USING utf8mb4) COLLATE utf8mb4_bin');
        });
