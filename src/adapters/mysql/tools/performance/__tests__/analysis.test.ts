...
describe('Performance Analysis Tools', () => {
    ...
    describe('createExplainTool', () => {
        ...
        it('should run EXPLAIN for query', async () => {
            ...
            const tool = createExplainTool(mockAdapter as unknown as MySQLAdapter);
            const result = await tool.handler({ query: 'SELECT * FROM users', format: 'TRADITIONAL' }, mockContext);

            expect(mockAdapter.executeReadQuery).toHaveBeenCalledWith('EXPLAIN SELECT * FROM users');
        ...
        it('should run EXPLAIN with TREE format', async () => {
            ...
            const tool = createExplainTool(mockAdapter as unknown as MySQLAdapter);
            const result = await tool.handler({ query: 'SELECT * FROM users', format: 'TREE' }, mockContext);

            expect(mockAdapter.executeReadQuery).toHaveBeenCalledWith('EXPLAIN FORMAT=TREE SELECT * FROM users');
            expect(result).toHaveProperty('plan');
        });
        ...
        it('should run EXPLAIN with JSON format', async () => {
            ...
            const tool = createExplainTool(mockAdapter as unknown as MySQLAdapter);
            const result = await tool.handler({ query: 'SELECT * FROM users', format: 'JSON' }, mockContext) as { plan: unknown };

            expect(mockAdapter.executeReadQuery).toHaveBeenCalledWith('EXPLAIN FORMAT=JSON SELECT * FROM users');
        ...
        it('should fall back to raw rows if JSON parsing fails or not string', async () => {
            ...
            mockAdapter.executeReadQuery.mockResolvedValue(createMockQueryResult(mockRows));
    ...
    describe('createExplainAnalyzeTool', () => {
        it('should create tool with correct definition', () => {
            const tool = createExplainAnalyzeTool(mockAdapter as unknown as MySQLAdapter);
        ...
        it('should run EXPLAIN ANALYZE', async () => {
            mockAdapter.executeReadQuery.mockResolvedValue(createMockQueryResult([{
                EXPLAIN: '-> Table scan on users  (actual time=0.05..0.10 rows=100 loops=1)'
            }]));

            const tool = createExplainAnalyzeTool(mockAdapter as unknown as MySQLAdapter);
            const result = await tool.handler({ query: 'SELECT * FROM users' }, mockContext);

            expect(mockAdapter.executeReadQuery).toHaveBeenCalledWith('EXPLAIN ANALYZE FORMAT=TREE SELECT * FROM users');
        ...
        it('should handle JSON format for EXPLAIN ANALYZE', async () => {
            ...
            const tool = createExplainAnalyzeTool(mockAdapter as unknown as MySQLAdapter);
            const result = await tool.handler({ query: 'SELECT 1', format: 'JSON' }, mockContext) as { analysis: unknown };

            expect(mockAdapter.executeReadQuery).toHaveBeenCalledWith('EXPLAIN ANALYZE FORMAT=JSON SELECT 1');
    ...
    describe('createSlowQueriesTool', () => {
        it('should query performance_schema for slow queries', async () => {
            ...
            const tool = createSlowQueriesTool(mockAdapter as unknown as MySQLAdapter);
    ...
    describe('createTableStatsTool', () => {
        ...
        it('should query table statistics', async () => {
            ...
            const tool = createTableStatsTool(mockAdapter as unknown as MySQLAdapter);
            const result = await tool.handler({ table: 'users' }, mockContext) as { stats: unknown };
        ...
        it('should return undefined stats if table not found', async () => {
            ...
            const tool = createTableStatsTool(mockAdapter as unknown as MySQLAdapter);
            const result = await tool.handler({ table: 'nonexistent' }, mockContext) as { stats: unknown };
    ...
    describe('createBufferPoolStatsTool', () => {
        it('should create tool with correct definition', () => {
            const tool = createBufferPoolStatsTool(mockAdapter as unknown as MySQLAdapter);
        ...
        it('should query buffer pool stats', async () => {
            ...
            expect(mockAdapter.executeReadQuery).toHaveBeenCalledWith('SELECT * FROM information_schema.INNODB_BUFFER_POOL_STATS');
    ...
    describe('createThreadStatsTool', () => {
        it('should create tool with correct definition', () => {
            const tool = createThreadStatsTool(mockAdapter as unknown as MySQLAdapter);
