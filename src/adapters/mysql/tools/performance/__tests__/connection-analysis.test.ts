  ...
  describe("mysql_detect_connection_spike", () => {
    ...
    it("should detect user concentration and long sleep", async () => {
      ...
      expect(result.data.warnings).toContainEqual(
        expect.stringContaining("Critical connection pressure"),
      );
      expect(result.data.warnings).toContainEqual(
        expect.stringContaining("sleeping for >5 minutes"),
      );
    ...
    it("should detect high connection pressure and 50+ sleep buildup", async () => {
      ...
      expect(result.data.warnings).toContainEqual(
        expect.stringContaining("High connection pressure"),
      );
      expect(result.data.warnings).toContainEqual(
        expect.stringContaining("60 total sleeping connections"),
      );
    });
