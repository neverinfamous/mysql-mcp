...
import { createMigrationRollbackTool, createMigrationHistoryTool, createMigrationStatusTool } from "../migration-query.js";
...
describe("Migration Query Tools", () => {
  ...
  describe("Migration Rollback Tool", () => {
    ...
    beforeEach(() => {
      tool = createMigrationRollbackTool(mockAdapter as unknown as MySQLAdapter);
    });
    ...
    it("should rollback a migration by id", async () => {
      mockAdapter.executeReadQuery.mockImplementation(async (query: string) => {
        if (query.includes("LIMIT 1")) {
          return createMockQueryResult([
            { id: 1, version: "1.0", status: "applied", rollback_sql: "DROP TABLE a" }
          ]);
        }
        return createMockQueryResult([]);
      });
      ...
      expect(executedQueries.some(q => q.includes("UPDATE") && q.includes("status = 'rolled_back'"))).toBe(true);
    });
    ...
    it("should handle dryRun", async () => {
      ...
      const result = await tool.handler({ version: "1.0", dryRun: true }, mockContext);
  ...
  describe("Migration History Tool", () => {
    ...
    it("should apply status filters", async () => {
      mockAdapter.executeReadQuery.mockImplementation(async (query: string, values: any[]) => {
        if (query.includes("SELECT m.id")) {
          expect(query).toContain("status = ?");
          expect(values).toContain("failed");
          return createMockQueryResult([{ id: 3, version: "3.0", status: "failed" }]);
        }
        return createMockQueryResult([]);
      });
  ...
  describe("Migration Status Tool", () => {
    ...
    it("should return initialized false if table is missing", async () => {
      mockAdapter.executeReadQuery.mockImplementation(async (query: string) => {
        if (query.includes("information_schema.TABLES")) return createMockQueryResult([{ table_exists: 0 }]);
        return createMockQueryResult([]);
      });
    ...
    it("should return aggregated status when table exists", async () => {
      mockAdapter.executeReadQuery.mockImplementation(async (query: string) => {
        ...
        if (query.includes("COUNT(*)")) return createMockQueryResult([{ total: 10, applied: 8, failed: 2 }]);
        if (query.includes("ORDER BY applied_at DESC LIMIT 1")) return createMockQueryResult([{ version: "2.0", applied_at: new Date() }]);
        if (query.includes("DISTINCT source_system")) return createMockQueryResult([{ source_system: "agent" }]);
        return createMockQueryResult([]);
      });
