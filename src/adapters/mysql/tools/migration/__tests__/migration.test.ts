...
import { createMigrationInitTool, createMigrationRecordTool, createMigrationApplyTool } from "../migration.js";
...
describe("Migration Core Tools", () => {
  ...
  describe("Migration Init Tool", () => {
    ...
    it("should initialize the tracking table", async () => {
      ...
      const sqlCall = mockAdapter.executeWriteQuery.mock.calls[0]?.[0] as string;
      expect(sqlCall).toContain("CREATE TABLE IF NOT EXISTS testdb._mcp_schema_versions");
    });
    ...
    it("should return correct metrics if table already exists", async () => {
      mockAdapter.executeReadQuery.mockImplementation(async (query: string) => {
        ...
        if (query.includes("COUNT(*)")) return createMockQueryResult([{ count: 5 }]);
        return createMockQueryResult([]);
      });
  ...
  describe("Migration Record Tool", () => {
    ...
    it("should record a migration without executing DDL", async () => {
      const result = await tool.handler({ version: "1.0", migrationSql: "CREATE TABLE a (id INT)" }, mockContext);
      ...
      expect(executedQueries.some(q => q.includes("INSERT INTO testdb._mcp_schema_versions"))).toBe(true);
      expect(executedQueries.some(q => q === "CREATE TABLE a (id INT)")).toBe(false);
    });
    ...
    it("should reject duplicate applied migrations", async () => {
      mockAdapter.executeReadQuery.mockImplementation(async (query: string) => {
        if (query.includes("SELECT id, version, status")) {
          // Simulate duplicate
          return createMockQueryResult([{ id: 1, version: "1.0", status: "applied" }]);
        }
        return createMockQueryResult([]);
      });
  ...
  describe("Migration Apply Tool", () => {
    ...
    it("should execute migration SQL and record it", async () => {
      const result = await tool.handler({ version: "1.0", migrationSql: "CREATE TABLE b (id INT)" }, mockContext);
    ...
    it("should record failure if migration throws", async () => {
      ...
      mockAdapter.executeWriteQuery.mockImplementation(async (query: string) => {
        if (query === "CREATE TABLE bad (id INT)") {
          throw new Error("Syntax error");
        }
      });

      const result = await tool.handler({ version: "1.0", migrationSql: "CREATE TABLE bad (id INT)" }, mockContext);
      ...
      const calls = mockAdapter.executeWriteQuery.mock.calls;
      const executedQueries = calls.map(c => c[0] as string);
      expect(executedQueries.some(q => q.includes("VALUES") && q.includes("'failed'"))).toBe(true);
    });
  });
});
