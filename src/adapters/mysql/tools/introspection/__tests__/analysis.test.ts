...
import { createConstraintAnalysisTool, createMigrationRisksTool } from "../analysis.js";
...
describe("Constraint Analysis Tool", () => {
  ...
  it("should detect missing primary keys", async () => {
    // Schema check
    mockAdapter.executeReadQuery.mockResolvedValueOnce(createMockQueryResult([{ SCHEMA_NAME: "testdb" }]));
    // Return tables without PKs
    mockAdapter.executeReadQuery.mockResolvedValueOnce(
      createMockQueryResult([{ schema_name: "testdb", table_name: "no_pk_table" }])
    );
    // Missing NOT NULL query
    mockAdapter.executeReadQuery.mockResolvedValueOnce(createMockQueryResult([]));

    const result = await tool.handler({ schema: "testdb", checks: ["missing_pk"] }, mockContext);
  ...
  it("should detect circular dependencies", async () => {
    ...
    const result = await tool.handler({ schema: "testdb", checks: ["circular_dependency"] }, mockContext);
  ...
  it("should detect missing NOT NULL columns", async () => {
    ...
    mockAdapter.executeReadQuery.mockResolvedValueOnce(
      createMockQueryResult([
        { schema_name: "testdb", table_name: "users", column_name: "email", type: "varchar(255)" }
      ])
    );

    const result = await tool.handler({ schema: "testdb", checks: ["missing_not_null"] }, mockContext);
...
describe("Migration Risks Tool", () => {
  ...
  it("should detect critical risk like DROP TABLE", async () => {
    const result = await tool.handler({ statements: ["DROP TABLE users;"] }, mockContext);
  ...
  it("should detect high risk like MODIFY COLUMN", async () => {
    const result = await tool.handler({ statements: ["ALTER TABLE users MODIFY COLUMN email VARCHAR(255) NOT NULL;"] }, mockContext);
  ...
  it("should detect medium risk like adding foreign key", async () => {
    const result = await tool.handler({ statements: ["ALTER TABLE users ADD CONSTRAINT fk_user FOREIGN KEY (id) REFERENCES other(id);"] }, mockContext);
  ...
  it("should detect low risk like CREATE TABLE without IF NOT EXISTS", async () => {
    const result = await tool.handler({ statements: ["CREATE TABLE users (id INT);"] }, mockContext);
