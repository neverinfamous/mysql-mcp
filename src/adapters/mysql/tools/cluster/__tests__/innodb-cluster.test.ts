...
describe('InnoDB Cluster Tools', () => {
    ...
    describe('createClusterStatusTool', () => {
        it('should return cluster status when InnoDB Cluster metadata exists', async () => {
            mockAdapter.executeQuery
                .mockResolvedValueOnce(createMockQueryResult([{ SCHEMA_NAME: 'mysql_innodb_cluster_metadata' }])) // Schema check
                .mockResolvedValueOnce(createMockQueryResult([{ cluster_name: 'myCluster' }])) // Cluster info
            ...
            const tool = createClusterStatusTool(mockAdapter as unknown as MySQLAdapter);
        ...
        it('should fallback to GR members when InnoDB Cluster metadata query fails', async () => {
            mockAdapter.executeQuery
                ...
                .mockResolvedValueOnce(createMockQueryResult([
                    { serverUuid: 'uuid1', address: 'host1:3306', memberState: 'ONLINE', memberRole: 'PRIMARY' }
                ])); // GR fallback

            const tool = createClusterInstancesTool(mockAdapter as unknown as MySQLAdapter);
    ...
    describe('createClusterRouterStatusTool', () => {
        it('should return unavailable message when router metadata query fails', async () => {
            mockAdapter.executeQuery.mockRejectedValue(new Error('Table does not exist'));

            const tool = createClusterRouterStatusTool(mockAdapter as unknown as MySQLAdapter);
            const result = await tool.handler({}, mockContext) as any;
