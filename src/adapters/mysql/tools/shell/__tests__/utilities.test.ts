...
describe('Shell Utilities Tools', () => {
    ...
    describe('mysqlsh_check_upgrade', () => {
        it('should run upgrade check and return JSON result', async () => {
            const successJson = JSON.stringify({ success: true, result: { status: 'OK' } });
            setupMockSpawn(`${successJson}\n`, '');

            const tool = createShellCheckUpgradeTool();
            const result = await tool.handler({ targetVersion: '8.0.35' }, mockContext) as any;
        ...
        it('should return raw output if JSON parsing fails but exit code is 0', async () => {
            ...
            const tool = createShellCheckUpgradeTool();
            const result = await tool.handler({ targetVersion: '8.0.0' }, mockContext) as any;
        ...
        it('should include both options when both provided', async () => {
            ...
            const tool = createShellCheckUpgradeTool();
            await tool.handler({ targetVersion: '9.0.0', outputFormat: 'TEXT' }, mockContext);
