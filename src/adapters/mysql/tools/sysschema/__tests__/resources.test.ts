...
describe('Sys Schema Resource Tools', () => {
    ...
    describe('createSysSchemaStatsTool', () => {
        it('should create tool with correct definition', () => {
            const tool = createSysSchemaStatsTool(mockAdapter as unknown as MySQLAdapter);
        ...
        it('should query schema stats (tables, indexes, auto-inc)', async () => {
            ...
            const tool = createSysSchemaStatsTool(mockAdapter as unknown as MySQLAdapter);
            const result = await tool.handler({}, mockContext) as { tableStatistics: unknown[], indexStatistics: unknown[], autoIncrementStatus: unknown[] };
        ...
        it('should handle null/undefined rows', async () => {
            mockAdapter.executeQuery.mockResolvedValue({ fields: [], rows: null } as any);
    ...
    describe('createSysInnoDBLockWaitsTool', () => {
        it('should create tool with correct definition', () => {
            const tool = createSysInnoDBLockWaitsTool(mockAdapter as unknown as MySQLAdapter);
        ...
        it('should query lock waits', async () => {
            ...
            const tool = createSysInnoDBLockWaitsTool(mockAdapter as unknown as MySQLAdapter);
            const result = await tool.handler({}, mockContext) as { hasContention: boolean };
    ...
    describe('createSysMemorySummaryTool', () => {
        it('should create tool with correct definition', () => {
            const tool = createSysMemorySummaryTool(mockAdapter as unknown as MySQLAdapter);
            expect(tool.name).toBe('mysql_sys_memory_summary');
        });

        it('should query memory stats (global and user)', async () => {
            mockAdapter.executeQuery
                .mockResolvedValueOnce(createMockQueryResult([{ event_name: 'memory/innodb/buf_buf_pool' }])) // Global
                .mockResolvedValueOnce(createMockQueryResult([{ user: 'root' }])); // User

            const tool = createSysMemorySummaryTool(mockAdapter as unknown as MySQLAdapter);
            const result = await tool.handler({}, mockContext) as { globalMemory: unknown[], memoryByUser: unknown[] };
