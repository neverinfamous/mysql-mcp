...
describe("Spatial Queries Tools", () => {
  ...
  describe("createSpatialDistanceTool", () => {
    ...
    it("should filter by maxDistance and use custom SRID", async () => {
      mockAdapter.executeReadQuery.mockImplementation(async (sql) => { if (sql.includes("information_schema.columns")) return createMockQueryResult([{ DATA_TYPE: "geometry", SRS_ID: 3857 }]); return createMockQueryResult([]); });
    ...
    it("should handle undefined rows result", async () => {
      ...
      mockAdapter.executeReadQuery.mockImplementation(async (sql) => { if (sql.includes("information_schema.columns")) return createMockQueryResult([{ DATA_TYPE: "geometry", SRS_ID: 4326 }]); return { fields: [], rows: null } as any; });
  ...
  describe("createSpatialDistanceSphereTool", () => {
    ...
    it("should query spherical distance", async () => {
      mockAdapter.executeReadQuery.mockImplementation(async (sql) => { if (sql.includes("information_schema.columns")) return createMockQueryResult([{ DATA_TYPE: "geometry", SRS_ID: 4326 }]); return createMockQueryResult([]); });
