...
const ContainsSchema = z.object({
    ...
    srid: z.number().default(4326).describe('SRID of the input geometry (default: 4326 for GPS coordinates)')
});
 ...
 */
export function createSpatialDistanceTool(adapter: MySQLAdapter): ToolDefinition {
    return {
        name: 'mysql_spatial_distance',
        title: 'MySQL Spatial Distance',
        description: 'Find rows within a certain distance from a point (Cartesian distance).',
        group: 'spatial',
 ...
 */
export function createSpatialDistanceSphereTool(adapter: MySQLAdapter): ToolDefinition {
    return {
        name: 'mysql_spatial_distance_sphere',
        title: 'MySQL Spherical Distance',
        description: 'Calculate distance on a sphere (for geographic coordinates). Returns distance in meters.',
        group: 'spatial',
        inputSchema: DistanceSchema,
        requiredScopes: ['read'],
        annotations: {
 ...
 */
export function createSpatialContainsTool(adapter: MySQLAdapter): ToolDefinition {
    return {
        name: 'mysql_spatial_contains',
        title: 'MySQL Spatial Contains',
        description: 'Find rows where the geometry is contained within a specified polygon.',
        group: 'spatial',
