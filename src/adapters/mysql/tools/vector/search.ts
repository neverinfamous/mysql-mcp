  ...
  return {
    ...
    handler: async (params: unknown, _context: RequestContext) => {
      try {
        ...
        const query = `SELECT ${selectCols}, DISTANCE(\`${column}\`, STRING_TO_VECTOR(?), '${metricLiteral}') as distance FROM \`${table}\` ${whereClause} ORDER BY distance ASC LIMIT ${validated.k}`.replace(/\s+/g, ' ').trim();
  ...
  return {
    ...
    handler: async (params: unknown, _context: RequestContext) => {
      try {
        ...
        const query = `SELECT ${selectCols}, DISTANCE(\`${column}\`, STRING_TO_VECTOR(?), '${metricLiteral}') as distance FROM \`${table}\` ${whereClause} HAVING distance <= ? ORDER BY distance ASC LIMIT ${validated.limit}`.replace(/\s+/g, ' ').trim();
