...
export const JsonSetSchema = z
  .preprocess(
    ...
    z.object({
      ...
      path: z.string().regex(/^\$((?:\.[a-zA-Z0-9_$]+)|(?:\."[^"]+")|(?:\[\s*\d+\s*\]))*$/, "Invalid JSON path expression (must start with $ and use valid path legs. Wildcards are not supported for modification)").optional(),
