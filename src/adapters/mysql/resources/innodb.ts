...
export function createInnodbResource(adapter: MySQLAdapter): ResourceDefinition {
    return {
        uri: 'mysql://innodb',
        name: 'InnoDB Status',
        ...
        handler: async (_uri: string, _context: RequestContext) => {
            ...
            for (const row of bufferPoolResult.rows ?? []) {
                bufferPool[row['Variable_name'] as string] = parseInt(row['Value'] as string, 10);
            }
            ...
            for (const row of opsResult.rows ?? []) {
                operations[row['Variable_name'] as string] = parseInt(row['Value'] as string, 10);
            }
            ...
            const hitRatio = readRequests > 0
                ? Math.round(((readRequests - reads) / readRequests) * 100 * 100) / 100
                : 100;
            ...
            return {
                buffer_pool: {
                    ...
                    instances: parseInt(config['innodb_buffer_pool_instances'] ?? '1', 10),
                    ...
                    pages: {
                        ...
                        data: pagesData,
                        dirty: pagesDirty,
                        dirty_percent: pagesTotal > 0 ? Math.round((pagesDirty / pagesTotal) * 100) : 0
                    },
