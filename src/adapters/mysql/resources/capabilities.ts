...
import { TOOL_GROUPS, META_GROUPS, getMetaGroupInfo, getToolGroupInfo } from '../../../filtering/ToolFilter.js';

export function createCapabilitiesResource(adapter: MySQLAdapter): ResourceDefinition {
    return {
        uri: 'mysql://capabilities',
        name: 'Server Capabilities',
        title: 'MySQL Server Capabilities',
        description: 'MySQL server version, extensions, and available tool categories',
        ...
        handler: async (_uri: string, _context: RequestContext) => {
            // Get server version
            const versionResult = await adapter.executeQuery('SELECT VERSION() as version');
            ...
            const features = {
                ...
                partitioning: true,
                replication: true,
                gtid: version.startsWith('5.6') || version.startsWith('5.7') || version.startsWith('8.')
            };
