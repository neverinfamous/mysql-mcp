...
export function createClusterResource(adapter: MySQLAdapter): ResourceDefinition {
    return {
        uri: 'mysql://cluster',
        name: 'Cluster Status',
        ...
        handler: async (_uri: string, _context: RequestContext) => {
            try {
                ...
                const groupName = typeof groupNameVal === 'string' ? groupNameVal : null;
                ...
                return {
                    ...
                    primary: primaryRow ? `${String(primaryRow['host'])}:${String(primaryRow['port'])}` : null
                };
            } catch {
                return {
                    groupReplicationEnabled: false,
                    message: 'Unable to retrieve cluster information',
                    suggestion: 'Group Replication may not be configured or insufficient privileges'
                };
            }
        }
    };
