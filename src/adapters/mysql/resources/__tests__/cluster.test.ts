...
describe('Cluster Resource', () => {
    ...
    it('should return cluster status when group replication is enabled', async () => {
        ...
        mockAdapter.executeQuery.mockResolvedValueOnce(createMockQueryResult([
            { Variable_name: 'group_replication_group_name', Value: 'aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee' }
        ]));

        // Mock replication_group_members
        mockAdapter.executeQuery.mockResolvedValueOnce(createMockQueryResult([
            { id: '1', host: 'host1', port: 3306, state: 'ONLINE', role: 'PRIMARY', version: '8.0.35' },
            { id: '2', host: 'host2', port: 3306, state: 'ONLINE', role: 'SECONDARY', version: '8.0.35' }
        ]));
        ...
        const resource = createClusterResource(mockAdapter as unknown as MySQLAdapter);
        const result = await resource.handler('mysql://cluster', mockContext) as any;
