/**
 * mysql-mcp — Audit Interceptor Tests
 *
...
function tempDir(): string {
  return join(
    tmpdir(),
    `mysql-audit-int-test-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
  );
}
...
describe("AuditInterceptor", () => {
  ...
  it("should log admin tool execution with category=admin", async () => {
    ...
    await interceptor.around(
      "mysql_execute_code",
      { code: "mysql.core.readQuery({sql: 'SELECT 1'})" },
      "req-003",
      async () => ({ result: 1 }),
    );
    await logger.flush();
    ...
    expect(entry.tool).toBe("mysql_execute_code");
  ...
  it("should measure duration", async () => {
    ...
    const interceptor = createAuditInterceptor(logger);
    await interceptor.around("mysql_write_query", {}, "req-007", async () => {
      // Simulate some work
  ...
  it("should log read-scoped tools when auditReads is enabled", async () => {
    ...
    expect(entry.tool).toBe("mysql_read_query");
  ...
  it("should use compact format for read entries (no args, user, scopes)", async () => {
    ...
    expect(entry.tool).toBe("mysql_list_tables");
    expect(entry.category).toBe("read");
    expect(entry.success).toBe(true);
    expect(entry.tokenEstimate).toBeGreaterThan(0);
  });

  it("should use logAs option to override recorded tool name", async () => {
    mockGetRequiredScope.mockReturnValue("write");
    mockGetAuthContext.mockReturnValue(undefined);
