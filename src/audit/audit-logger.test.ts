...
async function createTempDir(): Promise<string> {
  return mkdtemp(join(tmpdir(), "mysql-audit-test-"));
}
...
describe("AuditLogger", () => {
  ...
  it("should write JSONL entries to file", async () => {
    ...
    logger.log(fakeEntry({ tool: "mysql_create_table" }));
    await logger.close();
    ...
    expect(first.tool).toBe("mysql_write_query");
    expect(second.tool).toBe("mysql_create_table");
  });
