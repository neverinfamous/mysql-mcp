export const CORE_HELP = `# Core Tools (\`mysql_read_query\`, \`mysql_write_query\`, \`mysql_list_tables\`, etc.)
...
- **Strict Separation**: \`mysql_read_query\` strictly blocks write operations. \`mysql_write_query\` strictly blocks read-only operations (SELECT, SHOW, DESCRIBE, EXPLAIN) to prevent hallucinations where agents expect returned rows.
