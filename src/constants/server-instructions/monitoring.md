...
- **Status/Variables**: `mysql_show_status` and `mysql_show_variables` accept `like` for filtering (e.g., `like: "%connections%"`) and `limit` to cap rows (default: 30). Response includes `totalAvailable` and `limited: true` when truncated. RSA public key values in status output are automatically redacted.
