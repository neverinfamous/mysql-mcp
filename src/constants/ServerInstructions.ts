...
- **Create/Drop safety**: \`mysql_role_create\` supports \`ifNotExists: true\` (default) to skip if role exists. \`mysql_role_drop\` supports \`ifExists: true\` (default) to skip if role doesn't exist.
...
- **Graceful schema errors**: \`mysql_create_schema\` returns \`{ success: false, reason }\` when the schema already exists (without \`ifNotExists\`). With \`ifNotExists: true\`, returns \`{ success: true, skipped: true, reason: "Schema already exists" }\` for existing schemas. \`mysql_drop_schema\` returns \`{ success: false, reason }\` when the schema does not exist (without \`ifExists\`). With \`ifExists: true\`, returns \`{ success: true, skipped: true, reason: "Schema did not exist" }\` for nonexistent schemas.
...
- **Global variables**: \`proxysql_global_variables\` supports \`prefix\` filter (\`mysql\`, \`admin\`, or \`all\`) and \`like\` pattern for variable name matching. Use \`limit\` (default: 200) to control payload size. Variables containing passwords or credentials are automatically redacted.
- **Runtime status**: \`proxysql_runtime_status\` returns version and all admin variables. Sensitive variables (passwords, credentials) are automatically redacted.
