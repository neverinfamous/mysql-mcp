/**
 * mysql-mcp - Code Mode Sandbox Performance Benchmarks
 *
...
describe("Sandbox Execution", () => {
  ...
  bench(
    "execution with 22-group API bindings",
    async () => {
      ...
      await sandbox.execute(
        "const result = mysql.core.readQuery(); return result;",
        apiBindings,
      );
    },
    { iterations: 100, warmupIterations: 10, time: 5000 },
...
describe("Security Validation", () => {
  ...
  bench(
    ...
    () => {
      security.validateCode(
        'const result = await mysql.core.readQuery("SELECT 1");',
      );
    },
  ...
  bench(
    ...
    () => {
      ...
      for (let i = 0; i < 200; i++) {
        lines.push(
          `const result${String(i)} = await mysql.core.readQuery("SELECT * FROM table${String(i)} LIMIT 10");`,
        );
      }
