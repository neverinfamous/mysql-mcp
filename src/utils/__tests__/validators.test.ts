...
describe('validators', () => {
    describe('validateIdentifier', () => {
        ...
        it('should reject identifiers starting with numbers', () => {
            expect(() => validateIdentifier('123table', 'table'))
                .toThrow(ValidationError);
        });

        it('should reject identifiers with special characters', () => {
            expect(() => validateIdentifier('table-name', 'table'))
                .toThrow(ValidationError);
            expect(() => validateIdentifier('table name', 'table'))
                .toThrow(ValidationError);
            expect(() => validateIdentifier('table;drop', 'table'))
                .toThrow(ValidationError);
        });
        ...
        it('should reject identifiers exceeding 64 characters', () => {
            ...
            expect(() => validateIdentifier(longName, 'table'))
                .toThrow(ValidationError);
        });
    ...
    describe('validateQualifiedIdentifier', () => {
        ...
        it('should accept schema.table format', () => {
            expect(() => validateQualifiedIdentifier('mydb.users', 'table')).not.toThrow();
        });

        it('should reject invalid qualified names', () => {
            expect(() => validateQualifiedIdentifier('db.table.column', 'table'))
                .toThrow(ValidationError);
        });
    });
    ...
    describe('validateWhereClause', () => {
        ...
        it('should accept complex valid conditions', () => {
            expect(() => validateWhereClause("created_at > '2024-01-01' AND status = 'active'")).not.toThrow();
            expect(() => validateWhereClause("name LIKE '%test%'")).not.toThrow();
            expect(() => validateWhereClause('(a = 1 OR b = 2) AND c = 3')).not.toThrow();
        });

        it('should accept NULL and empty values', () => {
            expect(() => validateWhereClause('')).not.toThrow();
            expect(() => validateWhereClause(null as unknown as string)).not.toThrow();
        });

        it('should reject stacked queries', () => {
            expect(() => validateWhereClause('1=1; DROP TABLE users'))
                .toThrow(ValidationError);
