...
- **`mysql_event_alter` RENAME TO Clause Ordering** — Tool generated invalid SQL when combining `newName` with `body` (or other clauses). The `RENAME TO` clause was placed after `DO <body>`, but MySQL's `ALTER EVENT` syntax requires `RENAME TO` before `DO`. Reordered clause generation so `RENAME TO` is emitted before `DO`.
- **Code Mode `events.help()` Example** — `eventCreate` example in `mysql.events.help()` used a string schedule format (`schedule: 'EVERY 1 DAY'`) that does not match the required object format (`schedule: { type: 'RECURRING', interval: 1, intervalUnit: 'DAY' }`). Updated to use the correct object structure.
