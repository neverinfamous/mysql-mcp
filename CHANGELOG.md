...
- **`mysql_kill_query` Limited Error Catch** — The `try/catch` block only caught "Unknown thread id" errors, re-throwing all other MySQL errors (permission issues, connection failures) as raw MCP exceptions. Now catches all errors and returns `{ success: false, error: message }` for any failure
