...
- **Spatial GeoJSON Output Axis Order (P147)** — Fixed `mysql_spatial_buffer` and `mysql_spatial_intersection` returning GeoJSON coordinates in wrong order (`[lat, lon]` instead of `[lon, lat]`). MySQL's `ST_AsGeoJSON()` returns EPSG axis order for SRID 4326, but GeoJSON spec requires longitude-first. Added `fixGeoJsonAxisOrder()` helper that recursively swaps coordinate pairs in the output.
