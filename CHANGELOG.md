...
- **Cluster Tool Raw Error Leaks (7 Tools)** — All 5 GR tools (`mysql_gr_status`, `mysql_gr_members`, `mysql_gr_primary`, `mysql_gr_transactions`, `mysql_gr_flow_control`) and 2 InnoDB Cluster tools (`mysql_cluster_topology`, `mysql_cluster_switchover`) lacked outer `try/catch` blocks, causing raw MCP exceptions on unexpected query failures (connection loss, permission denied, etc.). All 7 now return structured error responses matching each tool's existing "not active" response shape
