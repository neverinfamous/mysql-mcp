...
async function runTest() {
  ...
  const server = spawn('node', ['dist/cli.js', '--transport', 'http', '--port', '35791'], {
    stdio: 'pipe',
    shell: false,
  ...
  for (let i = 0; i < 20; i++) {
    try {
      await fetch("http://localhost:35791/health");
      serverStarted = true;
      break;
    } catch {
      await delay(500);
  ...
  try {
    ...
    let res = await fetch("http://localhost:35791/health");
    ...
    res = await fetch("http://localhost:35791/mcp", {
      method: "POST",
      headers: { 
    ...
    res = await fetch("http://localhost:35791/health");
    body = await res.json();
    ...
    await fetch("http://localhost:35791/mcp", {
      method: "POST",
      headers: { 
        "Content-Type": "application/json", 
        "Accept": "application/json, text/event-stream",
