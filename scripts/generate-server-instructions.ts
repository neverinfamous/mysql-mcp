...
lines.push(" * - essential: Core quick access only (for token-constrained clients)");
...
lines.push(" * Contains quick access table, built-in tools, and base help pointer.");
lines.push(" */");
...
lines.push("/**");
lines.push(" * All group keys that have help content (for dynamic help pointer generation).");
lines.push(" */");
lines.push("const HELP_GROUP_KEYS: readonly string[] = " + JSON.stringify(groupHelpKeys));
lines.push("");
lines.push("/**");
...
lines.push("    const enabledHelpGroups = HELP_GROUP_KEYS.filter((k) => groups.has(k as ToolGroup))");
lines.push("    if (enabledHelpGroups.length === 0) {");
lines.push("        return '\\n\\nRead `mysql://help` for gotchas and critical usage patterns.'");
lines.push("    }");
lines.push("    return '\\n\\n## Help Resources\\n\\n' +");
lines.push("        'Read `mysql://help` for gotchas and critical usage patterns.\\n' +");
lines.push("        'Read `mysql://help/{group}` for: ' + enabledHelpGroups.join(', ') + '.'");
lines.push("}");
lines.push("");
lines.push("/**");
lines.push(" * Build active groups summary listing enabled group names with tool counts.");
lines.push(" */");
lines.push("function buildActiveGroupsSummary(groups: Set<ToolGroup>, enabledToolCount: number): string {");
...
lines.push("        instructions += buildActiveGroupsSummary(enabledGroups, enabledToolCount)");
lines.push("    }");
lines.push("");
