...
function auditDir(dir) {
    ...
    files.forEach(file => {
        ...
        // e.g. test-codemode-advanced-admin-control
        // or test-usability-admin-part1
        ...
        // Check if file is too long (over 200 lines) and print it out
        const lines = content.split('\n').length;
        if (lines > 200) {
            console.log(`[SPLIT CANDIDATE] ${file} has ${lines} lines (${content.length} bytes)`);
        }
    });
