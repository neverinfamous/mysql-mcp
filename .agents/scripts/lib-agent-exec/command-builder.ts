...
export function convertToWslPath(winPath: string): string {
  ...
  if (!winPath.startsWith('-') && winPath.includes('\\')) {
    if (/[{}`;]/.test(winPath)) return winPath;
...
export function buildCommand(payload: ExecPayload, cwd: string, isJson: boolean = false): { cmd: string; args: string[]; tempScriptPath: string | null; envOverrides: Record<string, string> } {
  ...
  if (!payload.bypassInterceptors) {
    const initialCmdBasename = payload.type === 'command' ? path.basename(payload.command).toLowerCase() : '';
    const initialCtx: ExecutionContext = { cmdBasename: initialCmdBasename, args: payload.args ? [...payload.args] : [], envOverrides, payload };
    systemInterceptor(initialCtx);
    if (payload.type === 'command') {
      payload.args = initialCtx.args;
    }
  }
  ...
  match(payload)
    .with({ type: 'command' }, (p) => {
      ...
      if (doubleQuoteMatch) {
        ...
        const tokens = tokenizeArgs(c);
        if (tokens.length > 1) {
          p.command = tokens[0];
          p.args = [...tokens.slice(1), ...(p.args || [])];
        }
      }
    })
    .with({ type: 'script' }, () => {})
  ...
  const parsedCmdArgs = match(payload)
    .with({ type: 'command' }, (p) => {
      ...
      if (p.target === 'wsl2') {
        ...
        localArgs = localArgs.map((a, idx) => {
          ...
          if (idx === patternExprIdx) return cleaned;
          return convertToWslPath(cleaned);
        });
      ...
      const hasCmdMeta = /[|&;<>$`{}()]/.test(localCmd) || localCmd.includes(' ');
      const hasArgMeta = localArgs.some(a => /[|&;<>]|\$\(|\$\{|`/.test(a));
      const hasShellMeta = hasCmdMeta || hasArgMeta;
      ...
      if (BUILT_INS.includes(localCmd.toLowerCase()) || (p.target === 'wsl2' && hasShellMeta)) {
        ...
        if (process.platform === 'win32' && p.target !== 'wsl2') {
          ...
          const shellOperators = ['&&', '||', '|', '|&', '>', '>>', '<', '<<', '<<<', ';', '2>', '1>', '2>&1', '&'];
          const originalCmd = [
            /[|&;<>$`{}()]/.test(localCmd) ? localCmd : escapeUnixArg(localCmd, false),
            ...localArgs.map(a => shellOperators.includes(a.trim()) ? a.trim() : escapeUnixArg(a, false))
          ].join(' ');
          localCmd = 'sh';
    ...
    .with({ type: 'script' }, (p) => {
      ...
      if (p.interpreter) {
        const interpLower = p.interpreter.toLowerCase();
        if (['ts', 'typescript'].includes(interpLower)) {
          localCmd = 'bun';
        } else if (['js', 'javascript'].includes(interpLower)) {
          localCmd = 'node';
        } else if (interpLower === 'py') {
          localCmd = 'python';
        } else if (interpLower === 'ps1') {
          localCmd = 'pwsh';
        } else if (interpLower === 'sh') {
          localCmd = 'bash';
        } else {
    ...
    .with({ type: 'eval' }, (p) => {
      ...
      let effectiveInterpreter = p.interpreter;
      if (p.target === 'wsl2' && effectiveInterpreter && (isPwsh(effectiveInterpreter) || isCmd(effectiveInterpreter))) {
        effectiveInterpreter = 'bash';
      }

      let ext = p.target === 'wsl2' ? '.sh' : '.ts';
      if (effectiveInterpreter) {
        const interp = effectiveInterpreter.toLowerCase();
        ext = match(interp)
          .when(i => i.includes('python') || i === 'py', () => '.py')
          .when(i => i.includes('pwsh') || i.includes('powershell') || i === 'ps1', () => '.ps1')
          .when(i => i.includes('bash') || i.includes('sh') || i.includes('zsh'), () => '.sh')
          .when(i => i.includes('cmd') || i.includes('bat'), () => '.bat')
          .when(i => i.includes('node') || i === 'js' || i === 'javascript', () => '.js')
          .when(i => i.includes('bun') || i === 'ts' || i === 'typescript', () => '.ts')
          .otherwise(() => ext);
      }
      ...
      try {
        ...
        if (ext === '.ps1') {
          ...
          codeToExecute = codeToExecute.replace(/\b([a-zA-Z]):[/\\][^\s"'`;&|)]*/g, (matchStr) => convertToWslPath(matchStr));
        }
      ...
      let localCmd = effectiveInterpreter || (p.target === 'wsl2' ? 'bash' : 'bun');
      const interpLower = localCmd.toLowerCase();
      if (['ts', 'typescript'].includes(interpLower)) {
        localCmd = 'bun';
      } else if (['js', 'javascript'].includes(interpLower)) {
        localCmd = 'node';
  ...
  if (unixUtilsForGlob.includes(cmdBasenameForGlob)) {
    let patternExprIdx = -1;
    if (['grep', 'grep.exe', 'egrep', 'fgrep', 'rg', 'rg.exe', 'sed', 'sed.exe', 'awk', 'awk.exe'].includes(cmdBasenameForGlob)) {
      patternExprIdx = args.findIndex(a => !a.startsWith('-'));
    }
    const hasGlobs = args.some((a, idx) => {
      if (idx === patternExprIdx) return false;
      return (a.includes('*') || a.includes('?')) && !a.startsWith('-');
    });
    if (hasGlobs) {
  ...
  if (ctx.cmdBasename && ctx.cmdBasename !== path.basename(cmd).toLowerCase()) {
    cmd = ctx.cmdBasename;
  }
  args = ctx.args;
  ...
  if (payload.type === 'command') {
    const isWinBatch = cmd.toLowerCase().endsWith('.bat') || cmd.toLowerCase().endsWith('.cmd') || (process.platform === 'win32' && ['pnpm', 'npm', 'yarn', 'npx'].includes(cmd.toLowerCase()));
  ...
  if (payload.target === 'wsl2') {
    ...
    if (/[a-zA-Z]:[/\\]/.test(wslCmd)) {
      ...
      wslCmd = 'python3';
    } else if (isPwsh(wslCmd) || isCmd(wslCmd)) {
      wslCmd = 'bash';
      if (tempScriptPath) {
        wslArgs = [convertToWslPath(tempScriptPath)];
      }
    }
