...
describe('System Interceptor: False Negatives and Positives', () => {
    ...
    it('should strip -R and composite -inR flags for ripgrep (rg)', () => {
        const payload: ExecPayload = { type: 'command', command: 'rg', args: ['-inR', 'pattern', 'src'] };
        const ctx = { cmdBasename: 'rg', args: payload.args as string[], envOverrides: {}, payload };
        systemInterceptor(ctx);
        expect(ctx.args).toEqual(['-in', 'pattern', 'src']);
    });

    it('should auto-heal grep-style --include/--exclude/--exclude-dir flags for ripgrep (rg)', () => {
        const payload: ExecPayload = { type: 'command', command: 'rg', args: ['--include', '*.ts', '--exclude=*.test.ts', '--exclude-dir', 'node_modules', 'pattern', 'src'] };
        const ctx = { cmdBasename: 'rg', args: payload.args as string[], envOverrides: {}, payload };
        systemInterceptor(ctx);
        expect(ctx.args).toEqual(['-g', '*.ts', '-g=!*.test.ts', '-g', '!node_modules/**', 'pattern', 'src']);
    });
});
