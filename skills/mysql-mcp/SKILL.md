...
- **Code Mode Safety**: Code Mode does **NOT** bypass destructive-operation safety gates. `DROP`, `TRUNCATE`, `DELETE` without `WHERE`, `ALTER TABLE ... DROP COLUMN`, `DROP DATABASE`, and `ALTER TABLE ... RENAME COLUMN` executed inside Code Mode scripts MUST still require explicit user confirmation before the script is submitted.
