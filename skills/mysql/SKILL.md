...
description: |
  ...
  Keywords: MySQL, MariaDB, InnoDB, my.cnf, mysql-mcp, STRICT_TRANS_TABLES
---
...
> [!NOTE]
> **Version Notice (2026)**: MySQL 8.0 reached EOL in April 2026. Actively target **8.4 LTS** or **9.x** for production environments. MariaDB has consolidated active Galera development and introduced native vector search.
...
- **DDL Destructive Ops**: `TRUNCATE TABLE`, `DROP TABLE`, and `DROP DATABASE` MUST require explicit user confirmation before execution. Never infer intent for these.
...
- **Connection Pooling**: Always assume and design for connection pooling. Code evaluating database connections should handle acquiring from and releasing to a pool, avoiding connection leaks. 
  ...
  - **Multiplexing**: Utilize **ProxySQL** to handle connection multiplexing and read/write splitting.
