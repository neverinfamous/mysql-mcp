...
- **Observability Constraints**:
  ...
  - **Duplicate Autodiscovery**: STRICTLY FORBIDDEN to mix static `conf.d/<check>.d/conf.yaml` configurations with Docker Compose `com.datadoghq.ad.*` labels. Use labels exclusively for containerized services.
- **WSL Context**: 
  ...
  - Datadog Agent containers running in WSL MUST include `pid: host` for accurate process collection. Compose v5+ rejects `cgroupns_mode: host` and `cgroup: host`; advise setting `"default-cgroupns-mode": "host"` in `/etc/docker/daemon.json` instead.
